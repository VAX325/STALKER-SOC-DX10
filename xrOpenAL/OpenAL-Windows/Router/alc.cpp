...
ALboolean NewSpecifierCheck(const ALCchar* specifier)
{
	...
	while (*list != 0)
	{
		if (strcmp((char*)list, (char*)specifier) == 0) { return AL_FALSE; }
...
ALvoid BuildDeviceSpecifierList()
{
	...
	{ // don't re-build lists if it's already been done once...
		...
		//
		if (GetLoadedModuleDirectory("OpenAL32.dll", dir[0], MAX_PATH)) { ++numDirs; }
...
ALvoid CleanDeviceSpecifierList()
{
	...
	while (strlen((const char*)list) > 0)
	{
		...
		{
			if (strstr(copyList, T2A("Generic Hardware")) != NULL) { advancePtr = FALSE; }
		}
		if (strstr(newListPtr, T2A("DirectSound")) != NULL)
		{
			if (strstr(copyList, T2A("Generic Software")) != NULL) { advancePtr = FALSE; }
		}
		if (strstr(newListPtr, "MMSYSTEM") != NULL)
		{
			if (strstr(copyList, T2A("Generic Software")) != NULL) { advancePtr = FALSE; }
		}
		if (advancePtr == TRUE) { newListPtr += strlen((const char*)newListPtr) + 1; }
...
ALboolean FillOutAlcFunctions(ALCdevice* device)
{
	...
	// alcGetEnumValue
	if (alcApi->alcGetProcAddress == NULL) { alcApi->alcGetProcAddress = (ALCAPI_GET_PROC_ADDRESS)alcGetProcAddress; }
	if (alcApi->alcIsExtensionPresent == NULL)
	{
		alcApi->alcIsExtensionPresent = (ALCAPI_IS_EXTENSION_PRESENT)alcIsExtensionPresent;
	}
	if (alcApi->alcGetEnumValue == NULL) { alcApi->alcGetEnumValue = (ALCAPI_GET_ENUM_VALUE)alcGetEnumValue; }
		...
		if (searchHandle != INVALID_HANDLE_VALUE)
		{
			while (TRUE)
			{
				...
				if (strstr(cmpName, "OPENAL32.DLL") == 0)
				{
					...
					if (skipSearch == false) { dll = LoadLibrary(searchName); }
				}
...
ALCAPI ALCboolean ALCAPIENTRY alcCloseDevice(ALCdevice* device)
{
	...
	if (IsBadReadPtr(device, sizeof(ALCdevice))) { return ALC_FALSE; }

	if (device == g_CaptureDevice) return g_CaptureDevice->AlcApi.alcCloseDevice(g_CaptureDevice->CaptureDevice);
	...
	if (device->InUse)
	{
		...
		while (entry)
		{
			...
			else { entry = alListIteratorNext(alContextList); }
		}
...
ALCAPI ALvoid ALCAPIENTRY alcDestroyContext(ALCcontext* context)
{
	...
	if (context == alCurrentContext) { alCurrentContext = 0; }
	...
	if (context->DllContext) { context->Device->AlcApi.alcDestroyContext(context->DllContext); }
...
ALCAPI ALCdevice* ALCAPIENTRY alcGetContextsDevice(ALCcontext* context)
{
	...
	alListAcquireLock(alContextList);
	if (alListMatchData(alContextList, context)) { ALCdevice = context->Device; }
...
ALCAPI ALenum ALCAPIENTRY alcGetEnumValue(ALCdevice* device, const ALCchar* ename)
{
	...
	if (alcEnums[i].ename) { return alcEnums[i].value; }
...
ALCAPI ALenum ALCAPIENTRY alcGetError(ALCdevice* device)
{
	...
	if (!device)
	{
		if (g_CaptureDevice == device) return errorCode = LastError;
...
ALCAPI ALvoid* ALCAPIENTRY alcGetProcAddress(ALCdevice* device, const ALCchar* fname)
{
	...
	if (alcFunctions[i].fname) { return alcFunctions[i].address; }
...
ALCAPI ALboolean ALCAPIENTRY alcIsExtensionPresent(ALCdevice* device, const ALCchar* ename)
{
	...
	if (alcExtensions[i].ename) { return ALC_TRUE; }
...
ALCAPI ALboolean ALCAPIENTRY alcMakeContextCurrent(ALCcontext* context)
{
	...
	if (context)
	{
		...
		if (contextSwitched && !context->AlApi.alGetProcAddress)
		{
			...
			if (!FillOutAlFunctions(context))
			{
				...
				else { alCurrentContext->Device->AlcApi.alcMakeContextCurrent(0); }
			}
		}
	}
	else
	...
	//
	if (contextSwitched) { alCurrentContext = context; }
...
ALCAPI ALCdevice* ALCAPIENTRY alcOpenDevice(const ALCchar* deviceName)
{
	...
	else { strncpy(newDeviceName, deviceName, 256); }
	...
	if (strcmp(newDeviceName, T2A("DirectSound")) == 0) { strcpy(newDeviceName, T2A("Generic Software")); }
	...
	if ((!dll) && (strcmp(newDeviceName, "Generic Software") == 0)) { dll = FindWrapper(); }

	// if device name is "Generic Hardware" and a device wasn't found, try to find a wrapper...
	if ((!dll) && (strcmp(newDeviceName, "Generic Hardware") == 0)) { dll = FindWrapper(); }
	...
	device->Dll = dll;
	if (!FillOutAlcFunctions(device)) { goto OpenDeviceFailed; }

	device->DllDevice = device->AlcApi.alcOpenDevice(newDeviceName);
	if (!device->DllDevice) { goto OpenDeviceFailed; }
...
ALCAPI ALvoid ALCAPIENTRY alcProcessContext(ALCcontext* context)
{
	...
	if (!context) { context = alCurrentContext; }

	EnterCriticalSection(&context->Lock);
	alListReleaseLock(alContextList);

	if (context->DllContext) { context->Device->AlcApi.alcProcessContext(context->DllContext); }
...
ALCAPI ALCvoid ALCAPIENTRY alcSuspendContext(ALCcontext* context)
{
	...
	if (!context) { context = (ALCcontext*)alCurrentContext; }
	...
	if (context->DllContext) { context->Device->AlcApi.alcSuspendContext(context->DllContext); }
	...
	if SUCCEEDED (hr)
	{
		...
		if SUCCEEDED (hr)
		{
			...
			if SUCCEEDED (hr)
			{
				...
				if (longName != NULL) { sprintf(longName, "%S", pv.pwszVal); }
				pPropertyStore->GetValue(PKEY_DeviceInterface_FriendlyName, &pv);
				if (shortName != NULL) { sprintf(shortName, "%S", pv.pwszVal); }
...
#endif
#endif // !defined(_WIN64)
		if ((shortName != NULL) && (strlen(outputInfo.szPname) <= len)) { strcpy(shortName, T2A(outputInfo.szPname)); }
	}
}
...
#endif
#endif // !defined(_WIN64)
		if ((shortName != NULL) && (strlen(inputInfo.szPname) <= len)) { strcpy(shortName, T2A(inputInfo.szPname)); }
	}
}
		...
		case ALC_DEFAULT_DEVICE_SPECIFIER:
			{
				while (TRUE)
				{
					...
					{ // test short device name (mixerDevice) next, with partial matches potentially...
						...
						if (dll)
						{
							if (acceptPartial == true) { strcpy(mixerDevice, actualName); }
		...
		case ALC_CAPTURE_DEFAULT_DEVICE_SPECIFIER:
			{
				while (TRUE)
				{
					...
					{ // look for short device (mixerDevice)
						...
						dll =
							FindDllWithMatchingSpecifier(_T("*oal.dll"), mixerDevice, acceptPartial, actualName, true);
						if (dll)
						{
							if (acceptPartial == true) { strcpy(mixerDevice, actualName); }
												   ...
												   ALCsizei buffersize)
{
	...
	if (!g_CaptureDevice)
	{
		...
		if (g_CaptureDevice)
		{
			...
			if (!deviceName) { strncpy(newDeviceName, alcGetString(0, ALC_CAPTURE_DEFAULT_DEVICE_SPECIFIER), 256); }
			else { strncpy(newDeviceName, deviceName, 256); }
			...
			if (g_CaptureDevice->Dll)
			{
				if (FillOutAlcFunctions(g_CaptureDevice))
				{
					...
					else { g_CaptureDevice->LastError = ALC_INVALID_DEVICE; }
				}
			}
		}
	}
	...
	{
		if (g_CaptureDevice->CaptureDevice) { return g_CaptureDevice; }
		else
		{
