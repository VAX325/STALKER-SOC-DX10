...
//--------------------------------------------------------------------------------------------------------------
SPass* CResourceManager::_CreatePass(ref_state& _state, ref_ps& _ps, ref_vs& _vs, ref_gs& _gs, ref_ctable& _ctable,
									 ref_texture_list& _T, ref_matrix_list& _M, ref_constant_list& _C)
{
	for (u32 it = 0; it < v_passes.size(); it++)
		if (v_passes[it]->equal(_state, _ps, _vs, _gs, _ctable, _T, _M, _C)) return v_passes[it];
...
SVS* CResourceManager::_CreateVS(LPCSTR _name)
{
	...
	else
	{
		...
		//_vs->vs				= NULL;
		//_vs->signature		= NULL;
		if (0 == stricmp(_name, "null")) { return _vs; }
		...
		_hr = ::Render->shader_compile(name, LPCSTR(fs->pointer()), fs->length(), NULL, &Includer, c_entry, c_target,
									   D3D10_SHADER_DEBUG |
										   D3D10_SHADER_PACK_MATRIX_ROW_MAJOR /*| D3DXSHADER_PREFER_FLOW_CONTROL*/,
...
#endif // DEBUG
	   //_hr = ::Render->shader_compile(name,LPCSTR(fs->pointer()),fs->length(), NULL, &Includer, c_entry, c_target,
	   // D3D10_SHADER_PACK_MATRIX_ROW_MAJOR | D3D10_SHADER_AVOID_FLOW_CONTROL /*| D3DXSHADER_PREFER_FLOW_CONTROL*/,
	   //&pShaderBuf, &pErrorBuf, NULL);
		FS.r_close(fs);
		...
		if (SUCCEEDED(_hr))
		{
			if (pShaderBuf)
			{
				_hr = HW.pDevice->CreateVertexShader(pShaderBuf->GetBufferPointer(), pShaderBuf->GetBufferSize(),
													 &_vs->vs);
				if (SUCCEEDED(_hr))
				{
					...
					if (SUCCEEDED(_hr) && pReflection)
					{
						...
						// CHK_DX( D3D10GetInputSignatureBlob(pShaderBuf->GetBufferPointer(),
						// pShaderBuf->GetBufferSize(), &_vs->signature) );
						ID3DBlob* pSignatureBlob;
						CHK_DX(D3D10GetInputSignatureBlob(pShaderBuf->GetBufferPointer(), pShaderBuf->GetBufferSize(),
														  &pSignatureBlob));
...
SPS* CResourceManager::_CreatePS(LPCSTR _name)
{
	...
	else
	{
		...
		// LPD3DXSHADER_CONSTANTTABLE	pConstants	= NULL;
		HRESULT _hr = S_OK;
		//_hr = ::Render->shader_compile	(name,data,size, NULL, &Includer, c_entry, c_target, D3DXSHADER_DEBUG |
		...
		_hr = ::Render->shader_compile(name, data, size, NULL, &Includer, c_entry, c_target,
									   D3D10_SHADER_DEBUG | D3D10_SHADER_PACK_MATRIX_ROW_MAJOR, &pShaderBuf, &pErrorBuf,
									   NULL);
#else
		_hr = ::Render->shader_compile(name, data, size, NULL, &Includer, c_entry, c_target,
									   D3D10_SHADER_PACK_MATRIX_ROW_MAJOR, &pShaderBuf, &pErrorBuf, NULL);
#endif // DEBUG
	   //_hr = ::Render->shader_compile(name,data,size, NULL, &Includer, c_entry, c_target,
	   // D3D10_SHADER_PACK_MATRIX_ROW_MAJOR | D3D10_SHADER_AVOID_FLOW_CONTROL, &pShaderBuf, &pErrorBuf, NULL); _hr =
	   // D3DXCompileShader		(text,text_size, NULL, &Includer, c_entry, c_target, D3DXSHADER_DEBUG |
	   // D3DXSHADER_PACKMATRIX_ROWMAJOR, &pShaderBuf, &pErrorBuf, NULL);
		xr_free(data);
		...
		if (SUCCEEDED(_hr))
		{
			if (pShaderBuf)
			{
				_hr = HW.pDevice->CreatePixelShader(pShaderBuf->GetBufferPointer(), pShaderBuf->GetBufferSize(),
													&_ps->ps);
				...
				((DWORD*)pShaderBuf->GetBufferPointer(),MAKEFOURCC('C','T','A','B'),&data,NULL); if (SUCCEEDED(_hr) &&
				data)
					{
		...
		if (FAILED(_hr)) Msg("Can't compile shader %s", _name);
...
SGS* CResourceManager::_CreateGS(LPCSTR name)
{
	...
	else
	{
		...
		_hr = ::Render->shader_compile(name, data, size, NULL, &Includer, c_entry, c_target,
									   D3D10_SHADER_PACK_MATRIX_ROW_MAJOR, &pShaderBuf, &pErrorBuf, NULL);
		//_hr = ::Render->shader_compile(name,data,size, NULL, &Includer, c_entry, c_target,
		// D3D10_SHADER_PACK_MATRIX_ROW_MAJOR | D3D10_SHADER_AVOID_FLOW_CONTROL, &pShaderBuf, &pErrorBuf, NULL);
		xr_free(data);
		...
		if (SUCCEEDED(_hr))
		{
			if (pShaderBuf)
			{
				_hr = HW.pDevice->CreateGeometryShader(pShaderBuf->GetBufferPointer(), pShaderBuf->GetBufferSize(),
													   &_gs->gs);
		...
		else { Msg("error is %s", (LPCSTR)pErrorBuf->GetBufferPointer()); }
		_RELEASE(pShaderBuf);
		_RELEASE(pErrorBuf);

		if (FAILED(_hr)) Msg("Can't compile shader %s", name);
