...
_DDS:
	{
		...
		if (IMG.MiscFlags & D3D10_RESOURCE_MISC_TEXTURECUBE)
			goto _DDS_CUBE;
		else
			goto _DDS_2D;
...
_DDS_2D:
		{
			...
			if (img_loaded_lod) { Reduce(LoadInfo.Width, LoadInfo.Height, IMG.MipLevels, img_loaded_lod); }
			...
			R_CHK2(D3DX10CreateTextureFromMemory(HW.pDevice, S->pointer(), S->length(), &LoadInfo, 0, &pTexture2D, 0),
				   fn);
			FS.r_close(S);
			mip_cnt = IMG.MipLevels;
			// OK
