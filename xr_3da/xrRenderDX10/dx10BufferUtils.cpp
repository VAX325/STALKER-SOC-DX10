...
namespace dx10BufferUtils
{
	...
	HRESULT IC CreateBuffer(ID3D10Buffer** ppBuffer, const void* pData, UINT DataSize, bool bImmutable,
							bool bIndexBuffer)
	{
	...
	DXGI_FORMAT ConvertVertexFormat(D3DDECLTYPE dx9FMT)
	{
		...
		{
			if (VertexFormatList[i].m_dx9FMT == dx9FMT) return VertexFormatList[i].m_dx10FMT;
		}
	...
	LPCSTR ConvertSemantic(D3DDECLUSAGE Semantic)
	{
		...
		for (int i = 0; i < arrayLength; ++i)
		{
			if (VertexSemanticList[i].m_dx9Semantic == Semantic) return VertexSemanticList[i].m_dx10Semantic;
		}
	...
	void ConvertVertexDeclaration(const xr_vector<D3DVERTEXELEMENT9>& declIn,
								  xr_vector<D3D10_INPUT_ELEMENT_DESC>& declOut)
	{
