...
BOOL R_constant_table::parseConstants(ID3D10ShaderReflectionConstantBuffer* pTable, u16 destination)
{
	...
	for (u32 i = 0; i < TableDesc.Variables; ++i)
	{
		...
		if (!C)
		{
			...
			R_constant_load& L = ((destination & RC_dest_pixel)    ? C->ps
								  : (destination & RC_dest_vertex) ? C->vs
																   : C->gs);
			L.index = r_index;
			L.cls = r_type;
			table.push_back(C);
...
BOOL R_constant_table::parseResources(ID3D10ShaderReflection* pReflection, int ResNum, u16 destination)
{
	for (int i = 0; i < ResNum; ++i)
	{
		...
		u16 r_index = u16(ResDesc.BindPoint + ((destination & RC_dest_pixel)    ? CTexture::rstPixel
											   : (destination & RC_dest_vertex) ? CTexture::rstVertex
																				: CTexture::rstGeometry));
...
BOOL R_constant_table::parse(void* _desc, u16 destination)
{
	...
	if (ShaderDesc.ConstantBuffers)
	{
		...
		for (u16 iBuf = 0; iBuf < ShaderDesc.ConstantBuffers; ++iBuf)
		{
			...
			if (pTable)
			{
				...
				updatedDest |= iBuf << ((destination & RC_dest_pixel)    ? RC_dest_pixel_cb_index_shift
										: (destination & RC_dest_vertex) ? RC_dest_vertex_cb_index_shift
																		 : RC_dest_geometry_cb_index_shift);

				//	Encode bind dest (pixel/vertex buffer) and bind point index
				u32 uiBufferIndex = iBuf;
				uiBufferIndex |= (destination & RC_dest_pixel)    ? CB_BufferPixelShader
								 : (destination & RC_dest_vertex) ? CB_BufferVertexShader
	...
	if (ShaderDesc.BoundResources) { parseResources(pReflection, ShaderDesc.BoundResources, destination); }
