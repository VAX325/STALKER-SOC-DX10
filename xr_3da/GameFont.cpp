...
CGameFont::CGameFont(LPCSTR section, u32 flags)
{
	...
	if (pSettings->line_exist(section, "interval")) SetInterval(pSettings->r_fvector2(section, "interval"));
}
...
{
	if (bCheckDevice && (!Device.b_is_Active)) return;
...
void __cdecl CGameFont::OutPrev(LPCSTR fmt, ...) { MASTER_OUT(TRUE, FALSE, FALSE, TRUE, 0.0f, 0.0f, -1.0f, fmt); };
...
float CGameFont::SizeOf_(const wide_char* wsStr)
{
	...
	if (len)
		for (unsigned int j = 1; j <= len; j++)
		{
			...
			if (IsNeedSpaceCharacter(wsStr[j])) fDelta += fXStep;
			X += fDelta;
		}
