...
BOOL CObject::net_Spawn(CSE_Abstract* data)
{
	...
	if (register_schedule()) shedule_register();
...
void CObject::net_Destroy()
{
	...
	if (register_schedule()) shedule_unregister();
...
void CObject::spatial_update(float eps_P, float eps_R)
{
	...
	else
	{
		...
		else
		{
			...
			bUpdate = TRUE;
			if (PositionStack.size() < 4) { PositionStack.push_back(SavedPosition()); }
			else
			{
...
{
	if (_destroy == (BOOL)Props.bDestroy) return;
	...
	if (_destroy)
	{
		...
		if (debug_destroy) Msg("cl setDestroy [%d][%d]", ID(), Device.dwFrame);
#endif
#ifdef MP_LOGGING
