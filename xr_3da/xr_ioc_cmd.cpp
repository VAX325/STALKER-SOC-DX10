...
void IConsole_Command::add_to_LRU(shared_str const& arg)
{
	...
	{
		m_LRU.push_back(arg);
		if (m_LRU.size() > LRU_MAX_COUNT) { m_LRU.erase(m_LRU.begin()); }
	}
}
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		if (Memory.debug_mode) { Memory.dbg_check(); }
		else { Msg("~ Run with -mem_debug options."); }
	}
};
#endif // DEBUG_MEMORY_MANAGER
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		if (!b_abs_name) FS.update_path(cfg_full_name, "$app_data_root$", cfg_full_name);

		if (strext(cfg_full_name)) *strext(cfg_full_name) = 0;
		strcat(cfg_full_name, ".ltx");

		BOOL b_allow = TRUE;
		if (FS.exist(cfg_full_name)) b_allow = SetFileAttributes(cfg_full_name, FILE_ATTRIBUTE_NORMAL);
...
void CCC_LoadCFG::Execute(LPCSTR args)
{
	...
	if (NULL == FS.exist(cfg_full_name)) FS.update_path(cfg_full_name, "$fs_root$", cfg_name);

	if (NULL == FS.exist(cfg_full_name)) strcpy_s(cfg_full_name, cfg_name);
	...
	if (F != NULL)
	{
		while (!F->eof())
		{
			...
			if (allow(str)) Console->Execute(str);
		}
		FS.r_close(F);
		Msg("[%s] successfully loaded.", cfg_full_name);
	}
	else { Msg("! Cannot open script file [%s]", cfg_full_name); }
}

CCC_LoadCFG_custom::CCC_LoadCFG_custom(LPCSTR cmd) : CCC_LoadCFG(cmd) { strcpy_s(m_cmd, cmd); };
...
class CCC_Start : public IConsole_Command
{
	...
	{
		dest[0] = 0;
		if (strstr(args, name)) sscanf(strstr(args, name) + xr_strlen(name), "(%[^)])", dest);
	}
	...
	void protect_Name_strlwr(LPSTR str)
	{
		...
		if (!name1 || !xr_strlen(name1)) { return; }
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		*/
		...
		if (!op_client[0] && strstr(op_server, "single")) strcpy_s(op_client, "localhost");
...
public:
	...
	virtual void Status(TStatus& S) { sprintf_s(S, sizeof(S), "%dx%d", psCurrentVidMode[0], psCurrentVidMode[1]); }
	virtual xr_token* GetToken() { return vid_mode_token; }
	virtual void Info(TInfo& I) { strcpy_s(I, sizeof(I), "change screen resolution WxH"); }
};
//-----------------------------------------------------------------------
...
public:
	...
	virtual void Save(IWriter* F)
	{
		// fill_render_mode_list	();
		tokens = vid_quality_token;
		if (!strstr(Core.Params, "-r2")) { inherited::Save(F); }
	}
...
public:
	...
	virtual void Info(TInfo& I) { sprintf_s(I, sizeof(I), "hide console"); }
};
