...
BOOL CAviPlayerCustom::Load(char* fname)
{
	...
	if (hmmioFile == NULL) { return FALSE; }
	...
	if (m_aviIC == 0) { return FALSE; }

	// Проинитить декомпрессор
	if (ICERR_OK != ICDecompressBegin(m_aviIC, &m_biInFormat, &m_biOutFormat)) { return FALSE; }
...
VOID CAviPlayerCustom::PreRoll(DWORD dwFrameNum)
{
	...
	res = ICDecompress(m_aviIC, ICDECOMPRESS_PREROLL | ICDECOMPRESS_HURRYUP, &m_biInFormat,
					   ...
					   m_pDecompressedBuf);
	if (ICERR_OK != res && ICERR_DONTDRAW != res) { R_ASSERT(0); }
...
{
	if (0 == m_dwFirstFrameOffset) m_dwFirstFrameOffset = Device.dwTimeContinual - 1;
