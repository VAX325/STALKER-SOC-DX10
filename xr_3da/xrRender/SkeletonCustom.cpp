...
LPCSTR CKinematics::LL_BoneName_dbg(u16 ID)
{
	...
	for (_I = bone_map_N->begin(); _I != _E; ++_I)
		if (_I->second == ID) return *_I->first;
	return 0;
}
...
CKinematics::~CKinematics()
{
	...
	if (m_lod)
	{
		...
		{
			if (lod_kinematics->m_is_original_lod) { lod_kinematics->Release(); }
		}
...
void CKinematics::LL_SetBoneVisible(u16 bone_id, BOOL val, BOOL bRecursive)
{
	...
	if (!visimask.is(mask)) { bone_instances[bone_id].mTransform.scale(0.f, 0.f, 0.f); }
	else { CalculateBones_Invalidate(); }
...
void CKinematics::LL_SetBonesVisible(u64 mask)
{
	...
	{
		u64 bm = u64(1) << b;
		if (mask & bm) { visimask.set(bm, TRUE); }
		else
		{
							  ...
							  ref_shader shader, float size)
{
	...
	for (u16 k = 0; k < LL_BoneCount(); k++)
	{
		...
		{
			Fobb& obb = cache_obb[k];
			if (CDB::TestSphereOBB(test_sphere, obb)) test_bones.push_back(k);
		}
	}
	...
	for (u32 wm_idx = 0; wm_idx < wallmarks.size(); wm_idx++)
	{
		...
		{
			if (wm_idx < wallmarks.size() - 1) wm = wallmarks.back();
			wallmarks.pop_back();
			break;
		}
	}
...
int CKinematics::LL_GetBoneGroups(xr_vector<xr_vector<u16>>& groups)
{
	...
	for (u16 bone_idx = 0; bone_idx < (u16)bones->size(); bone_idx++)
	{
		...
		for (u32 child_idx = 0; child_idx < children.size(); child_idx++)
		{
			if (!B->child_faces[child_idx].empty()) { groups[child_idx].push_back(bone_idx); }
		}
	}
	return groups.size();
}
