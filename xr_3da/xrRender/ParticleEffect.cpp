...
BOOL CParticleEffect::Compile(CPEDef* def)
{
	...
	if (m_Def)
	{
		...
		// time limit
		if (m_Def->m_Flags.is(CPEDef::dfTimeLimit)) m_fElapsedLimit = m_Def->m_fTimeLimit;
	}
...
// Render
//------------------------------------------------------------------------------
void CParticleEffect::Copy(IRenderVisual*) { FATAL("Can't duplicate particle system - NOT IMPLEMENTED"); }
			...
			for (u32 i = 0; i < p_cnt; i++)
			{
				...
				if (m_Def->m_Flags.is(CPEDef::dfAlignToPath))
				{
					...
					else
					{
						...
						else { FillSprite(pv, m.pos, dir, lt, rb, r_x, r_y, m.color, m.rot.x); }
					}
				}
				else
				{
