...
bool dxRenderDeviceRender::Create(HWND hWnd, u32& dwWidth, u32& dwHeight, float& fWidth_2, float& fHeight_2,
								  bool move_window)
{
...
void dxRenderDeviceRender::overdrawBegin()
{
	...
	else { CHK_DX(HW.pDevice->SetRenderState(D3DRS_STENCILZFAIL, D3DSTENCILOP_INCRSAT)); } // ZB access
#endif //	USE_DX10
}
...
dxRenderDeviceRender::DeviceState dxRenderDeviceRender::GetDeviceState()
{
	...
	if (FAILED(_hr))
	{
		...
		if (D3DERR_DEVICELOST == _hr) return dsLost;

		// Check if the device is ready to be reset
		if (D3DERR_DEVICENOTRESET == _hr) return dsNeedReset;
	}
#endif //	USE_DX10
