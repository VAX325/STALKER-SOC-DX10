...
std::pair<LPCSTR, LPCSTR> CKinematicsAnimated::LL_MotionDefName_dbg(MotionID ID)
{
	...
	for (_I = s_mots.motion_map()->begin(); _I != _E; ++_I)
		if (_I->second == ID.idx) return std::make_pair(*_I->first, *s_mots.id());
...
static void dump_blend(CKinematicsAnimated* K, CBlend& B, u32 index)
{
	...
	Msg("state: %s, playing: %s, stop_at_end_callback: %s ", name_blend_type(B.blend), name_bool(B.playing),
...
{
	if (LL_PartBlendsCount(bone_part_id) <= n) return 0;
...
void CKinematicsAnimated::LL_FadeCycle(u16 part, float falloff, u8 mask_channel /*= (1<<0)*/)
{
	...
	{
		CBlend& B = *Blend[I];
		if (!(mask_channel & (1 << B.channel))) continue;
...
{
	if (GetBlendDestroyCallback()) GetBlendDestroyCallback()->BlendDestroy(B);
	B.blend = CBlend::eFREE_SLOT;
...
void CKinematicsAnimated::LL_UpdateTracks(float dt, bool b_force, bool leave_blends)
{
	...
	for (u16 part = 0; part < MAX_PARTS; part++)
	{
		...
		{
			CBlend& B = *(*I);
			if (!b_force && B.dwFrame == Device.dwFrame) continue;
...
void CKinematicsAnimated::LL_BuldBoneMatrixDequatize(const CBoneData* bd, u8 channel_mask, SKeyTable& keys)
{
	...
	for (BI = Blend.begin(); BI != Blend.end(); BI++)
	{
		...
		if (!(channel_mask & (1 << B->channel))) continue;
	...
	for (u16 j = 0; MAX_CHANNELS > j; ++j)
		if (channels.rule(j).extern_ == animation::add) keys_substruct(keys.keys[j], BK[j], keys.chanel_blend_conts[j]);
}
...
void CKinematicsAnimated::LL_BoneMatrixBuild(CBoneInstance& bi, const Fmatrix* parent, const SKeyTable& keys)
{
	...
	for (u16 j = 0; MAX_CHANNELS > j; ++j)
	{
		if (j != 0 && keys.chanel_blend_conts[j] == 0) continue;
...
#ifdef DEBUG
	if (!check_scale(RES)) { VERIFY(check_scale(bi.mTransform)); }
