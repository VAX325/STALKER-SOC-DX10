...
void SimulatorStates::record(glState& state)
{
	for (u32 it = 0; it < States.size(); it++)
	{
		...
		{
			case 0:
				state.UpdateRenderState(S.v1, S.v2);
				break;
			// case 1: VERIFY(!"Texture enviroment not supported");	break;
			case 2:
				state.UpdateSamplerState(S.v1, S.v2, S.v3);
				break;
		}
	}
}
...
void SimulatorStates::UpdateDesc(D3D10_SAMPLER_DESC descArray[D3D10_COMMONSHADER_SAMPLER_SLOT_COUNT],
								 bool SamplerUsed[D3D10_COMMONSHADER_SAMPLER_SLOT_COUNT], int iBaseSamplerIndex) const
{
	...
	for (u32 it = 0; it < States.size(); it++)
	{
		...
		if (S.type == 2)
		{
			...
			if ((iSamplerIndex >= D3D10_COMMONSHADER_SAMPLER_SLOT_COUNT) || iSamplerIndex < 0) continue;
			...
			switch (S.v2)
			{
				...
				case XRDX10SAMP_ANISOTROPICFILTER:
					if (S.v3) desc.Filter = (D3D10_FILTER)(desc.Filter | FilterAnisotropic);
				...
				case XRDX10SAMP_COMPARISONFILTER:
					if (S.v3) desc.Filter = (D3D10_FILTER)(desc.Filter | FilterComparison);
