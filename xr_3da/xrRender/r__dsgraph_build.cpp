...
#if RENDER != R_R1
	if (val_recorder) { val_recorder->push_back(pVisual->vis.box); }
#endif
}
		...
		case MT_PARTICLE_GROUP:
			{
				...
				for (PS::CParticleGroup::SItemVecIt i_it = pG->items.begin(); i_it != pG->items.end(); i_it++)
				{
					...
					for (xr_vector<IRenderVisual*>::iterator pit = I._children_related.begin();
						 pit != I._children_related.end(); pit++)
						add_leafs_Dynamic((dxRender_Visual*)*pit);
					for (xr_vector<IRenderVisual*>::iterator pit = I._children_free.begin();
		...
		case MT_SKELETON_RIGID:
			{
				...
				if (_use_lod) { add_leafs_Dynamic(pV->m_lod); }
				else
				{
		...
		case MT_PARTICLE_GROUP:
			{
				...
				for (PS::CParticleGroup::SItemVecIt i_it = pG->items.begin(); i_it != pG->items.end(); i_it++)
				{
					...
					if (fcvPartial == VIS)
					{
						...
						for (xr_vector<IRenderVisual*>::iterator pit = I._children_related.begin();
							 pit != I._children_related.end(); pit++)
							add_Dynamic((dxRender_Visual*)*pit, planes);
						for (xr_vector<IRenderVisual*>::iterator pit = I._children_free.begin();
							 pit != I._children_free.end(); pit++)
							add_Dynamic((dxRender_Visual*)*pit, planes);
					}
