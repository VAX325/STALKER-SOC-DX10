...
void CBlender_Screen_SET::Save(IWriter& fs)
{
	...
	fs.w(&I, sizeof(I));
	I.ID = 6;
	strcpy_s(I.str, "MUL_2X (B^D)");
	fs.w(&I, sizeof(I));
	I.ID = 7;
	strcpy_s(I.str, "SET (2r)");
	fs.w(&I, sizeof(I));
	I.ID = 8;
	strcpy_s(I.str, "BLEND (2r)");
	fs.w(&I, sizeof(I));
	I.ID = 9;
	strcpy_s(I.str, "BLEND (4r)");
	fs.w(&I, sizeof(I));
...
void CBlender_Screen_SET::Compile(CBlender_Compile& C)
{
	...
	if (oBlend.IDselected == 6)
	{
		...
		if (oClamp.value) C.i_dx10Address(iSmp, D3DTADDRESS_CLAMP);
	}
	else
	{
		if (9 == oBlend.IDselected)
		...
		C.r_dx10Texture("s_base", C.L_textures[0]);
		int iSmp = C.r_dx10Sampler("smp_base");
		if ((oClamp.value) && (iSmp != u32(-1))) C.i_dx10Address(iSmp, D3DTADDRESS_CLAMP);
	}
