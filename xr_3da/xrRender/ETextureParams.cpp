...
void STextureParams::Load(IReader& F)
{
	...
	if (F.find_chunk(THM_CHUNK_TEXTURE_TYPE)) { type = (ETType)F.r_u32(); }
	...
	switch (type)
	{
		...
		case ttNormalMap:
			flags.set(flImplicitLighted | flBinaryAlpha | flAlphaBorder | flColorBorder | flFadeToColor |
						  flFadeToAlpha | flDitherColor | flDitherEachMIPLevel | flBumpDetail,
					  FALSE);
...
BOOL STextureParams::similar(STextureParams& tp1, xr_vector<AnsiString>& sel_params)
{
	...
	for (; it != it_e; ++it)
	{
		...
		if (par_name == "Type") { res = (type == tp1.type); }
		else if (par_name == "Source\\Width") { res = (width == tp1.width); }
		else if (par_name == "Source\\Height") { res = (height == tp1.height); }
		else if (par_name == "Source\\Alpha") { res = (HasAlpha() == tp1.HasAlpha()); }
		else if (par_name == "Format") { res = (fmt == tp1.fmt); }
		else if (par_name == "MipMaps\\Enabled")
		{
			res = (flags.test(flGenerateMipMaps) == tp1.flags.test(flGenerateMipMaps));
		}
		else if (par_name == "MipMaps\\Filter") { res = (mip_filter == tp1.mip_filter); }
		else if (par_name == "Bump\\Mode") { res = (bump_mode == tp1.bump_mode); }
		else if (par_name == "Bump\\Texture") { res = (bump_name == tp1.bump_name); }
		...
		else if (par_name == "Details\\Scale") { res = (fsimilar(detail_scale, tp1.detail_scale)); }
		else if (par_name == "Material\\Base") { res = (material == tp1.material); }
		else if (par_name == "Material\\Weight") { res = (fsimilar(material_weight, tp1.material_weight)); }
		else if (par_name == "Flags\\Binary Alpha")
		{
			res = (flags.test(flBinaryAlpha) == tp1.flags.test(flBinaryAlpha));
		}
		else if (par_name == "Flags\\Dither") { res = (flags.test(flDitherColor) == tp1.flags.test(flDitherColor)); }
		...
		else if (par_name == "Fade\\Delay 'n' MIP") { res = (fade_delay == tp1.fade_delay); }
		else if (par_name == "Fade\\% of color to fade in") { res = (fade_amount == tp1.fade_amount); }
		else if (par_name == "Fade\\Color") { res = (fade_color == tp1.fade_color); }
		else if (par_name == "Fade\\Alpha") { res = (color_get_A(fade_color) == color_get_A(tp1.fade_color)); }
		...
		else if (par_name == "Border\\Color") { res = (border_color == tp1.border_color); }
		else if (par_name == "Bump\\Special NormalMap") { res = (ext_normal_map_name == tp1.ext_normal_map_name); }
