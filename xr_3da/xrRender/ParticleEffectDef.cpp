...
{
	if (*m_ShaderName && *m_TextureName) m_CachedShader.create(*m_ShaderName, *m_TextureName);
}
		...
		do
		{
			...
			if (dist >= EPS)
			{
				...
				if (g_pGameLevel->ObjectSpace.RayPick(m.posB, dir, dist, RT, RQ, NULL))
				{
					...
					pick_cnt++;
					if (cb && (pick_cnt == 1))
						if (!cb(owner, m, pt, n)) break;
					if (m_Flags.is(dfCollisionDel)) { ParticleManager()->RemoveParticle(owner->m_HandleEffect, i); }
					else
					{
						...
						if (vt.length2() <= m_fCollideSqrCutoff) { m.vel = vt - vn * m_fCollideResilience; }
						else { m.vel = vt * m_fCollideOneMinusFriction - vn * m_fCollideResilience; }
...
BOOL CPEDef::Load(IReader& F)
{
	...
	if (version != PED_VERSION) return FALSE;
	...
	{
		if (F.find_chunk(PED_CHUNK_ALIGN_TO_PATH)) { F.r_fvector3(m_APDefaultRotation); }
	}
...
BOOL CPEDef::Load2(CInifile& ini)
{
	...
	if (m_Flags.is(dfTimeLimit)) { m_fTimeLimit = ini.r_float("timelimit", "value"); }
	...
	if (m_Flags.is(dfVelocityScale)) { m_VelocityScale = ini.r_fvector3("velocity_scale", "value"); }

	if (m_Flags.is(dfAlignToPath)) { m_APDefaultRotation = ini.r_fvector3("align_to_path", "default_rotation"); }
#ifdef _EDITOR
	if (pCreateEAction)
	{
...
void CPEDef::Save2(CInifile& ini)
{
	...
	if (m_Flags.is(dfTimeLimit)) { ini.w_float("timelimit", "value", m_fTimeLimit); }
	...
	if (m_Flags.is(dfVelocityScale)) { ini.w_fvector3("velocity_scale", "value", m_VelocityScale); }

	if (m_Flags.is(dfAlignToPath)) { ini.w_fvector3("align_to_path", "default_rotation", m_APDefaultRotation); }
#ifdef _EDITOR
