...
void CModelPool::DeleteInternal(dxRender_Visual*& V, BOOL bDiscard)
{
	...
	if (!V) return;
	V->Depart();
	if (bDiscard || bForceDiscard) { Discard(V, TRUE); }
	else
	{
		//
...
void CModelPool::Delete(dxRender_Visual*& V, BOOL bDiscard)
{
	...
	else { DeleteInternal(V, bDiscard); }
	V = NULL;
}
...
void CModelPool::Render(dxRender_Visual* m_pVisual, const Fmatrix& mTransform, int priority, bool strictB2F,
						float m_fLOD)
{
	// render visual
		...
		case MT_SKELETON_RIGID:
			{
				...
				{
					IKinematics* pV = dynamic_cast<IKinematics*>(m_pVisual);
					VERIFY(pV);
