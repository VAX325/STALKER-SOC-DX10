...
ENGINE_API CInifile* pGameIni = nullptr; // declared in pch_engine.h
...
#define VIRT_MESSAGE_SIZE 512
	...
	if ((dwPhysMemInMB > 500) && ((dwPageFileInMB + dwPhysMemInMB) > 2500)) return 0;

	hApp = GetModuleHandle(NULL);

	if (!LoadString(hApp, RC_VIRT_MEM_ERROR, pszError, VIRT_ERROR_SIZE)) return 0;

	if (!LoadString(hApp, RC_VIRT_MEM_TEXT, pszMessage, VIRT_MESSAGE_SIZE)) return 0;
...
public:
	...
	{
		if (false) Msg("* sound: %d [%3.2f,%3.2f,%3.2f]", u32(Device.dwFrame), VPUSH(Device.vCameraPosition));
...
public:
	KeyFilter()
	{
		...
		if (screensaverState) SystemParametersInfo(SPI_SETSCREENSAVEACTIVE, FALSE, NULL, 0);
		stickyKeysFlags = 0;
		filterKeysFlags = 0;
		toggleKeysFlags = 0;
		stickyKeys = {0, 0};
		filterKeys = {0, 0, 0, 0, 0, 0};
		toggleKeys = {0, 0};
	...
	~KeyFilter()
	{
		...
		if (screensaverState) SystemParametersInfo(SPI_SETSCREENSAVEACTIVE, TRUE, NULL, 0);
...
{
#ifdef DEBUG
	if (strstr(Core.Params, "-slowdown")) { thread_spawn(slowdownthread, "slowdown", 0, 0); }
...
void Startup()
{
	...
	if (startupCmd) Console->Execute(startupCmd + 1);
	...
	Engine.Event.Dump();
	destroyInput();
	if (!g_bBenchmark && !g_SASH.IsRunning()) destroySettings();
	LALib.OnDestroy();
...
static BOOL CALLBACK LogoWndProc(HWND hw, UINT msg, WPARAM wp, LPARAM lp)
{
	switch (msg)
	{
		...
		case WM_CLOSE:
			...
			break;
		case WM_COMMAND:
			if (LOWORD(wp) == IDCANCEL) DestroyWindow(hw);
			break;
		default:
			return FALSE;
	}
...
int APIENTRY WinMain_impl(HINSTANCE hInstance, HINSTANCE hPrevInstance, char* lpCmdLine, int nCmdShow)
{
...
#ifndef DEDICATED_SERVER
	if (!strstr(lpCmdLine, "--skipmemcheck") && IsOutOfVirtualMemory()) return 0;
	...
	if (presenceMutex == NULL)
	{
		...
		if (presenceMutex == NULL) return 2;
	}
	else
	{
...
#ifndef DEDICATED_SERVER
	if (strstr(Core.Params, "-gl")) { Console->Execute("renderer renderer_gl"); }
	else if (strstr(Core.Params, "-r3")) { Console->Execute("renderer renderer_r3"); }
	else if (strstr(Core.Params, "-r2.5")) { Console->Execute("renderer renderer_r2.5"); }
	else if (strstr(Core.Params, "-r2a")) { Console->Execute("renderer renderer_r2a"); }
	else if (strstr(Core.Params, "-r2")) { Console->Execute("renderer renderer_r2"); }
	else
	{
...
int StackOverflowFilter(DWORD exceptionCode)
{
...
{
	static const char* textureNames[] = {"texture800", "texture", "texture1600"};
...
void _InitializeFont(CGameFont*& F, LPCSTR section, u32 flags)
{
	...
	else { F->Initialize("font", fontTextureName); }
...
void CApplication::OnEvent(EVENT E, u64 P1, u64 P2)
{
	...
	else if (E == eStart)
	{
		...
		if (!opServer || !strlen(opServer) ||
			((strstr(opServer, "/dm") || strstr(opServer, "/deathmatch") || strstr(opServer, "/tdm") ||
			  strstr(opServer, "/teamdeathmatch") || strstr(opServer, "/ah") || strstr(opServer, "/artefacthunt") ||
...
void CApplication::destroy_loading_shaders() { m_pRender->destroy_loading_shaders(); }
...
void CApplication::LoadTitleInt(LPCSTR str)
{
	...
	Msg("* phase cmem: %d K", Memory.mem_usage() / 1024);
	Log(app_title);
	if (g_pGamePersistent->GameType() == 1 && strstr(Core.Params, "alife")) { max_load_stage = 17; }
	else { max_load_stage = 14; }
	LoadDraw();
}
...
void CApplication::Level_Append(LPCSTR folder)
{
	...
	if (FS.exist("$game_levels$", level) && FS.exist("$game_levels$", ltx) && FS.exist("$game_levels$", geom) &&
...
void CApplication::Level_Scan()
{
	...
	if (false) R_ASSERT(folder && folder->size());
