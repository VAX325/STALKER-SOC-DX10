...
void CRenderTarget::phase_combine()
{
	...
	if (m_bHasActiveVolumetric) phase_combine_volumetric();
	...
	if (RImplementation.o.dx10_msaa)
	{
		...
		HW.pDevice->ResolveSubresource(rt_Generic_0_r->pTexture->surface_get(), 0,
									   rt_Generic_0->pTexture->surface_get(), 0, DXGI_FORMAT_R8G8B8A8_UNORM);
	}
	...
	{
		if (PP_Complex)
			u_setrt(rt_Generic, 0, 0, 0); // LDR RT
		else
			u_setrt(Device.dwWidth, Device.dwHeight, HW.pBaseRT, NULL, NULL, HW.pBaseZB);
	}
	else
