...
void CRenderTarget::phase_ssao()
{
	...
	// low/hi RTs
	if (!RImplementation.o.dx10_msaa) { u_setrt(rt_ssao_temp, 0, 0, 0 /*HW.pBaseZB*/); }
	else { u_setrt(rt_ssao_temp, 0, 0, 0 /*RImplementation.Target->rt_MSAADepth->pZRT*/); }
...
void CRenderTarget::phase_downsamp()
{
	...
	if (RImplementation.o.ssao_half_data) set_viewport(HW.pDevice, float(Device.dwWidth), float(Device.dwHeight));
}
