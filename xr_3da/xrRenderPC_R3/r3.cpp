...
void CRender::create()
{
	...
	*/
	...
	if (o.ssao_hbao || o.ssao_hdao) o.ssao_opt_data = true;
	...
	if (o.dx10_msaa)
	{
		...
		{
			if (ps_r3_msaa_atest) o.dx10_msaa_alphatest = MSAA_ATEST_DX10_0_ATOC;
		}
	}
	...
	dxRenderDeviceRender::Instance().Resources->RegisterConstantSetup("pos_decompression_params",
																	  &binder_pos_decompress_params);
	dxRenderDeviceRender::Instance().Resources->RegisterConstantSetup("pos_decompression_params2",
																	  &binder_pos_decompress_params2);
								...
								void* _ppConstantTable)
{
	...
	if (o.dx10_msaa)
	{
		...
		/*
		VERIFY( !o.dx10_msaa_alphatest || (o.dx10_msaa_alphatest&&o.dx10_msaa_opt) ||
		(o.dx10_msaa_alphatest&&o.dx10_msaa_alphatest_atoc)
	...
	// HRESULT		_result	= D3D10CompileShader(pSrcData,SrcDataLen,NULL,
	// defines,pInclude,pFunctionName,pTarget,Flags,ppShader,ppErrorMsgs,ppConstantTable);

	HRESULT _result =
		D3DX10CompileFromMemory(pSrcData, SrcDataLen,
	...
	if (SUCCEEDED(_result) && o.disasm)
	{
		...
		// D3DX10DisassembleShader(code->GetBufferPointer(), code->GetBufferSize(), FALSE, 0, &disasm ); // originally
		// used
		D3DXDisassembleShader(LPDWORD(code->GetBufferPointer()), FALSE, 0, &disasm);
		string_path dname;
		strconcat(sizeof(dname), dname, "disasm\\", name,
				  ('v' == pTarget[0])   ? ".vs"
				  : ('p' == pTarget[0]) ? ".ps"
										: ".gs");
