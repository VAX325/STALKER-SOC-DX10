...
#ifdef DEBUG
	if (warn_on_not_found) Msg("! scheduled object %s tries to unregister but is not registered", *O->shedule_Name());
#endif // DEBUG
...
void CSheduler::Unregister(ISheduled* A)
{
	...
	if (m_processing_now)
	{
		if (internal_Unregister(A, A->shedule.b_RT, false)) return;
	}
...
//			}
		...
		if (execTime > 15) { Msg("* xrSheduler: too much time consumed by object [%s] (%dms)", _obj_name, execTime); }
#endif // DEBUG
