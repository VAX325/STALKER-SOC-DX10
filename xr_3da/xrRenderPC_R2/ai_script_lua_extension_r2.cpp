...
{
#ifndef ENGINE_BUILD
	if (!psAI_Flags.test(aiLua)) return (0);
#endif
						  ...
						  LPCSTR caNameSpaceName)
{
	...
	{
#ifdef DEBUG
		if (!bfPrintOutput(tpLuaVM, caScriptName, l_iErrorCode)) vfPrintError(tpLuaVM, l_iErrorCode);
#endif
		return (false);
	}
	return (true);
...
{
	if (xr_strlen(namespace_name) && !bfGetNamespaceTable(tpLuaVM, namespace_name)) return (false);
...
luabind::object Script::lua_namespace_table(CLuaVirtualMachine* tpLuaVM, LPCSTR namespace_name)
{
	...
	for (;;)
	{
		if (!xr_strlen(S)) return (lua_namespace);
		LPSTR I = strchr(S, '.');
		if (!I) return (lua_namespace[S]);
		*I = 0;
