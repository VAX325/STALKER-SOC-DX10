...
template <>
void property_collection<ambient::effect_container_type, ambient>::display_name(u32 const& item_index,
																				LPSTR const& buffer,
...
template <>
void property_collection<ambient::sound_container_type, ambient>::display_name(u32 const& item_index,
																			   LPSTR const& buffer,
																			   u32 const& buffer_size)
{
	strcpy_s(buffer, buffer_size, m_container[item_index]->id().c_str());
...
void ambient::load(CInifile& ambients_config, CInifile& sound_channels_config, CInifile& effects_config,
...
void ambient::fill(editor::property_holder_collection* collection)
{
	...
	m_property_holder->add_property("effects", "effects", "this option is resposible for maximum effects",
									m_effects_collection);
	m_property_holder->add_property("sound channels", "sounds", "this option is resposible for sound channels",
									m_sounds_collection);
}
