...
{
	if (Console) Console->Execute("vid_restart");
}

void engine_impl::pause(bool const& value)
{
	if (value == !!Device.Paused()) return;
...
{
	if (value == m_input_captured) return;
...
void engine_impl::weather(LPCSTR value)
{
	...
	if (environment.CurrentWeatherName._get() == new_weather_id._get()) return;
		...
		if ((*i)->m_identifier._get() == new_frame_id._get())
		{
			...
			if (environment.Current[0]->exec_time < environment.Current[1]->exec_time)
			{
				if (environment.GetGameTime() > environment.Current[0]->exec_time)
					if (environment.GetGameTime() < environment.Current[1]->exec_time) set_time = false;
			}
			else
			{
				if (environment.GetGameTime() > environment.Current[0]->exec_time) set_time = false;
			}
			if (set_time)
			{
...
LPCSTR engine_impl::current_weather_frame()
{
	...
	if (!g_pGamePersistent->Environment().Current[0]) return ("");
...
void engine_impl::track_frame(float const& time)
{
	...
	CEnvironment& environment = g_pGamePersistent->Environment();
	if (!environment.Current[0]) return;

	if (!environment.Current[1]) return;
	...
	else
	{
		...
		current_time = start_time + time * (stop_time - start_time);
		if (current_time >= 24.f * 60.f * 60.f) current_time -= 24.f * 60.f * 60.f;
	}
...
{
	if (!g_pGamePersistent) return (0.f);

	CEnvironment& environment = g_pGamePersistent->Environment();
	if (!environment.Current[0]) return (0.f);
...
{
	CEnvironment& environment = g_pGamePersistent->Environment();
	if (!environment.Current[0]) return (0);
...
{
	CEnvironment& environment = g_pGamePersistent->Environment();
	if (!environment.CurrentEnv) return (0);
...
{
	CEnvironment& environment = g_pGamePersistent->Environment();
	if (!environment.Current[1]) return (0);
...
void engine_impl::weather_time_factor(float const& value_raw)
{
	...
	if (g_pGameLevel)
		g_pGameLevel->SetEnvironmentGameTimeFactor(iFloor(g_pGamePersistent->Environment().GetGameTime() * 1000.f),
												   value);

	if (g_pGamePersistent) g_pGamePersistent->Environment().fTimeFactor = value;
}

float engine_impl::weather_time_factor()
{
	if (!g_pGamePersistent) return (1.f);
