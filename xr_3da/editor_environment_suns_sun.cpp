...
void sun::fill(editor::property_holder_collection* collection)
{
	...
	properties->add_property("id", "common", "this option is resposible for sun identifier", m_id.c_str(),
							 string_getter, string_setter);
	properties->add_property("use", "sun", "this option is resposible for sun usage", m_use, m_use);
	properties->add_property("ignore color", "sun", "this option is resposible for sun ignore color", m_ignore_color,
							 m_ignore_color);
