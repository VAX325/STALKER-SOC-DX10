...
void CInput::KeyUpdate()
{
	...
	for (u32 i = 0; i < dwElements; i++)
	{
		...
		if (!KBState[key])
		{
			...
			if (key == DIK_TAB && (iGetAsyncKeyState(DIK_RMENU) || iGetAsyncKeyState(DIK_LMENU))) b_alt_tab = true;
		}
	}
	...
	if (b_alt_tab) SendMessage(Device.m_hWnd, WM_SYSCOMMAND, SC_MINIMIZE, 0);
...
BOOL CInput::iGetAsyncKeyState(int dik) { return !!KBState[dik]; }

BOOL CInput::iGetAsyncBtnState(int btn) { return !!mouseState[btn]; }

constexpr auto dimofs_x_c = 0L;        // DIMOFS_X
constexpr auto dimofs_y_c = 4L;        // DIMOFS_Y
constexpr auto dimofs_z_c = 8L;        // DIMOFS_Z
constexpr auto dimofs_button0_c = 12L; // DIMOFS_BUTTON0
constexpr auto dimofs_button1_c = 13L; // DIMOFS_BUTTON1
constexpr auto dimofs_button2_c = 14L; // DIMOFS_BUTTON2
constexpr auto dimofs_button3_c = 15L; // DIMOFS_BUTTON3
constexpr auto dimofs_button4_c = 16L; // DIMOFS_BUTTON4
constexpr auto dimofs_button5_c = 17L; // DIMOFS_BUTTON5
constexpr auto dimofs_button6_c = 18L; // DIMOFS_BUTTON6
constexpr auto dimofs_button7_c = 19L; // DIMOFS_BUTTON7
void CInput::MouseUpdate()
{
	...
	if (mouseState[0] && mouse_prev[0]) { cbStack.back()->IR_OnMouseHold(0); }

	if (mouseState[1] && mouse_prev[1]) { cbStack.back()->IR_OnMouseHold(1); }

	if (mouseState[2] && mouse_prev[2]) { cbStack.back()->IR_OnMouseHold(2); }
	if (dwElements)
	{
...
void CInput::iCapture(IInputReceiver* p)
{
	...
	// change focus
	if (!cbStack.empty()) cbStack.back()->IR_OnDeactivate();
	cbStack.push_back(p);
...
{
	if (CurrentIR()) CurrentIR()->IR_OnActivate();
...
{
	if (CurrentIR()) CurrentIR()->IR_OnDeactivate();
