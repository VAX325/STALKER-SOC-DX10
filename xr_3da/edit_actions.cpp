...
namespace text_editor
{
	...
	{
		if (m_previous_action) { m_previous_action->on_key_press(control); }
	}
	...
	void type_pair::on_key_press(line_edit_control* const control)
	{
		...
		if (m_translate)
		{
			...
			if (control->get_key_state(ks_Shift)) { c = c_shift; }
		}
		else
		{
			c = m_char;
			if (control->get_key_state(ks_Shift)) { c = m_char_shift; }
		}
	...
	key_state_base::key_state_base(key_state state) { m_state = state; }

	key_state_base::~key_state_base() {}

	void key_state_base::on_key_press(line_edit_control* const control) { control->set_key_state(m_state, true); }
