							...
							collide::ray_cache* cache, CObject* ignore_object)
{
	...
	if (tgt & rqtStatic)
	{
		...
		{
			// 0. similar query???
			if (cache->similar(start, dir, range)) { return cache->result; }
...
{
#ifdef DEBUG
	if (R.range < EPS || !_valid(R.range)) Debug.fatal(DEBUG_INFO, "Invalid RayQuery range passed: %f.", R.range);
#endif
	// initialize query
	r_dest.r_clear();
	r_temp.r_clear();
