...
namespace text_editor
{
	...
	{
		if (m_actions[dik]) { xr_delete(m_actions[dik]); }
	...
	void line_edit_control::on_key_press(int dik)
	{
		...
		if (m_actions[dik]) { m_actions[dik]->on_key_press(this); }
		// ===========
		if (dik == DIK_LCONTROL || dik == DIK_RCONTROL) { m_mark = false; }

		m_edit_str[m_buffer_size - 1] = 0;
		clamp_cur_pos();

		add_inserted_text();
		if (m_mark && (!get_key_state(ks_Shift) || !empty_inserted())) { m_select_start = m_cur_pos; }
		compute_positions();
	...
	void line_edit_control::on_frame()
	{
		...
		if (m_last_changed_frame + 1 < Device.dwFrame) { m_need_update = false; }
	...
	void line_edit_control::compute_positions()
	{
		m_p1 = m_cur_pos;
		m_p2 = m_cur_pos;
		if (m_unselected_mode) { return; }

		if (m_cur_pos > m_select_start) { m_p1 = m_select_start; }
		else if (m_cur_pos < m_select_start) { m_p2 = m_select_start; }
	}
	...
	void remove_spaces(PSTR str) // in & out
	{
		...
		while (b < str_size)
		{
			...
			i += (b - a);
			if (i < str_size) { new_str[i] = ' '; }
			++b;
			++i;
		}
		--i;
		if (i < str_size) { strncpy_s(str, str_size, new_str, i); }
	}
