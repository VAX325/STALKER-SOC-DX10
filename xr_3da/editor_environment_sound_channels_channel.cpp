...
template <>
void property_collection<channel::sound_container_type, channel>::display_name(u32 const& item_index,
																			   LPSTR const& buffer,
...
void channel::fill(editor::property_holder_collection* collection)
{
	...
	m_property_holder->add_property("minimum distance", "properties",
									"this option is resposible for minimum distance (in meters)", m_sound_dist.x,
									m_sound_dist.x);
	m_property_holder->add_property("maximum distance", "properties",
									"this option is resposible for maximum distance (in meters)", m_sound_dist.y,
									m_sound_dist.y);
	...
	m_property_holder->add_property("sounds", "properties", "this option is resposible for sound sources",
									m_collection);
}
