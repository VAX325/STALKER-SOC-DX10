...
void CPartition::load(IKinematics* V, LPCSTR model_name)
{
	...
	for (u32 i = 0; i < MAX_PARTS; ++i)
	{
		...
		if (S.Data.size()) { P[i].bones.clear_not_free(); }
		for (; it != it_e; ++it)
		{
			const CInifile::Item& I = *it;
			if (I.first == part_name) { P[i].Name = I.second; }
			else
			{
...
BOOL motions_value::load(LPCSTR N, IReader* data, vecBones* bones)
{
	...
	else { Debug.fatal(DEBUG_INFO, "Old skinned model version unsupported! (%s)", N); }
...
const motion_marks::interval* motion_marks::pick_mark(const float& t) const
{
	...
	{
		const interval& I = (*it);
		if (I.first <= t && I.second >= t) return &I;
...
bool motion_marks::is_mark_between(float const& t0, float const& t1) const
{
	...
	for (; i != e; ++i)
	{
		...
		if ((*i).first == t0) return (true);

		if ((*i).first > t0)
		{
			if ((*i).second <= t1) return (true);

			if ((*i).first <= t1) return (true);

			return (false);
		}

		if ((*i).second < t0) continue;

		if ((*i).second == t0) return (true);
...
float motion_marks::time_to_next_mark(float time) const
{
	...
	for (; i != e; ++i)
	{
		...
		if (dist > 0.f && dist < result_dist) result_dist = dist;
	}
	return result_dist;
}
