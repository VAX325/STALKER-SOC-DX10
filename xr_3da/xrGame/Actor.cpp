...
void CActor::reinit()
{
	...
	m_pUsableObject = NULL;
	if (!g_dedicated_server) memory().reinit();
...
void CActor::reload(LPCSTR section)
{
	...
	if (!g_dedicated_server) memory().reload(section);
...
void CActor::Load(LPCSTR section)
{
	...
	if (GameID() == GAME_SINGLE) OnDifficultyChanged();
	//////////////////////////////////////////////////////////////////////////
...
{
	auto shit_obj = SHit(P, dir, who, element, p_in_object_space, impulse, hit_type);
	m_pPhysics_support->in_Hit(shit_obj, !g_Alive());
}
		...
		default:
			{
				...
				if (HDS.hit_type == ALife::eHitTypeWound_2 && Check_for_BackStab_Bone(HDS.bone()))
				{
					...
					{
						game_PlayerState* ps = Game().GetPlayerByGameID(ID());
						if (!ps || !ps->testFlag(GAME_PLAYER_FLAG_INVINCIBLE)) m_bWasBackStabbed = true;
					}
				};
...
#pragma todo("Dima to Dima : forward-back bone impulse direction has been determined incorrectly!")
		MotionID motion_ID =
			m_anims->m_normal
				.m_damage[iFloor(pK->LL_GetBoneInstance(element).get_param(1) +
...
void CActor::Die(CObject* who)
{
	...
	if (IsGameTypeSingle()) { start_tutorial("game_over"); }
	xr_delete(m_sndShockEffector);
}

void CActor::SwitchOutBorder(bool new_border_state)
{
	if (new_border_state) { callback(GameObject::eExitLevelBorder)(lua_game_object()); }
	else
	{
...
void CActor::UpdateCL()
{
	...
	if (m_holder) m_holder->UpdateEx(currentFOV());
	...
	if (g_Alive()) CStepManager::update();
	...
	if (m_sndShockEffector)
	{
		if (this == Level().CurrentViewEntity())
		{
			...
			if (!m_sndShockEffector->InWork() || !g_Alive()) xr_delete(m_sndShockEffector);
		}
		else
			xr_delete(m_sndShockEffector);
	}
...
void CActor::shedule_Update(u32 DT)
{
	...
	if (GameID() == GAME_SINGLE) GameTaskManager().UpdateTasks();
	...
	//------------------------------------------------
	{
		...
		if (!Level().IsDemoPlay())
		{
			...
			if (g_bAutoClearCrouch) mstate_wishful &= ~mcCrouch;
			//-----------------------------------------------------
		}
	}
	else
	{
		...
		if (NET.size())
		{
			...
			g_Orientate(mstate_real, dt);
			g_Physics(NET_SavedAccel, NET_Jump, dt);
			if (!m_bInInterpolation) g_cl_ValidateMState(dt, mstate_wishful);
			g_SetAnimation(mstate_real);
	...
	if (this == Level().CurrentViewEntity()) { UpdateMotionIcon(mstate_real); };
	...
	if (this == Level().CurrentControlEntity() && !g_dedicated_server)
	{
		...
		if (!g_Alive() && m_BloodSnd._feedback()) m_BloodSnd.stop();
	}

	// если в режиме HUD, то сама модель актера не рисуется
	if (!character_physics_support()->IsRemoved()) setVisible(!HUDview());
...
void CActor::renderable_Render()
{
	...
	if (!HUDview()) { CInventoryOwner::renderable_Render(); }
}
...
{
	if (!m_pPhysicsShell) { character_physics_support()->movement()->SetPosition(transform.c); }
	// else m_phSkeleton->S
}
...
void CActor::OnItemDrop(CInventoryItem* inventory_item)
{
	...
	if (artefact && artefact->m_eItemPlace == eItemPlaceBelt) MoveArtefactBelt(artefact, false);
}
...
void CActor::OnItemDropUpdate()
{
	...
	for (; I != E; ++I)
		if (!(*I)->IsInvalid() && !attached(*I)) attach(*I);
}
...
void CActor::OnItemRuck(CInventoryItem* inventory_item, EItemPlace previous_place)
{
	...
	CArtefact* artefact = smart_cast<CArtefact*>(inventory_item);
	if (artefact && previous_place == eItemPlaceBelt) MoveArtefactBelt(artefact, false);
}
void CActor::OnItemBelt(CInventoryItem* inventory_item, EItemPlace previous_place)
{
	...
	CArtefact* artefact = smart_cast<CArtefact*>(inventory_item);
	if (artefact) MoveArtefactBelt(artefact, true);
}
...
{
	CUIMotionIcon& motion_icon = HUD().GetUI()->UIMainIngameWnd->MotionIcon();
	if (mstate_rl & mcClimb) { motion_icon.ShowState(CUIMotionIcon::stClimb); }
	else
	{
...
bool CActor::can_attach(const CInventoryItem* inventory_item) const
{
	...
	if (!item || /*!item->enabled() ||*/ !item->can_be_attached()) return (false);
	...
	if (attached(inventory_item->object().cNameSect())) return false;
