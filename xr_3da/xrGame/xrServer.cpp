...
void xrServer::client_Destroy(IClient* C)
{
	...
	for (u32 I = 0; I < net_Players.size(); I++)
	{
		if (net_Players[I] == C)
		{
			...
			if (!g_sv_Client_Reconnect_Time || !C->flags.bVerified) { xr_delete(C); }
			else
			{
...
#ifdef BATTLEYE
	if (g_pGameLevel) { Level().battleye_system.UpdateServer(this); }
#endif // BATTLEYE
}
...
u32 xrServer::OnMessage(NET_Packet& P, ClientID sender) // Non-Zero means broadcasting with "flags" as returned
{
	...
	switch (type)
	{
		...
		case M_SPAWN:
			{
				if (CL->flags.bLocal) Process_spawn(P, sender);
		...
		case M_CHANGE_LEVEL:
			{
				if (game->change_level(P, sender)) { SendBroadcast(BroadcastCID, P, net_flags(TRUE, TRUE)); }
				VERIFY(verify_entities());
			}
			break;
		case M_SAVE_GAME:
		...
		case M_PLAYER_FIRE:
			{
				if (game) game->OnPlayerFire(sender, P);
			}
			break;
		...
		case M_BATTLEYE:
			{
#ifdef BATTLEYE
				if (g_pGameLevel) { Level().battleye_system.ReadPacketServer(sender.value(), &P); }
#endif // BATTLEYE
			}
	}
...
bool xrServer::CheckAdminRights(const shared_str& user, const shared_str& pass, string512 reason)
{
	...
	if (FS.exist(fn))
	{
		...
		if (ini.line_exist("radmins", user.c_str()))
		{
			...
			else { strcpy(reason, "Access denied. Wrong password."); }
		}
		else
...
void xrServer::entity_Destroy(CSE_Abstract*& P)
{
	...
	if (P->owner && P->owner->owner == P) P->owner->owner = NULL;

	P->owner = NULL;
	if (!ai().get_alife() || !P->m_bALifeControl) { F_entity_Destroy(P); }
}
...
void xrServer::Server_Client_Check(IClient* CL)
{
	...
	{
		if (!CL->flags.bConnected) { SV_Client = NULL; };
		clients_Unlock();
		return;
	};
...
void xrServer::OnChatMessage(NET_Packet* P, xrClientData* CL)
{
	...
	for (u32 client = 0; client < net_Players.size(); ++client)
	{
		...
		if (Cps->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD) && !ps->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD)) continue;
		SendTo(Client->ID, *P);
	};
};
