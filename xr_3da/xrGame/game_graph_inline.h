						 ...
						 const _LOCATION_ID E[GameGraph::LOCATION_TYPE_COUNT]) const
{
	for (int i = 0; i < GameGraph::LOCATION_TYPE_COUNT; ++i)
		if ((M[i] != E[i]) && (255 != M[i])) return (false);
	return (true);
}
...
IC float CGameGraph::distance(const _GRAPH_ID tGraphID0, const _GRAPH_ID tGraphID1) const
{
	...
	begin(tGraphID0, i, e);
	for (; i != e; ++i)
		if (value(tGraphID0, i) == tGraphID1) return (edge_weight(i));
...
IC const CGameGraph::CVertex* CGameGraph::vertex(const u32& vertex_id) const { return (m_nodes + vertex_id); }
...
IC const GameGraph::SLevel& GameGraph::CHeader::level(LPCSTR level_name) const
{
	...
	for (; I != E; ++I)
		if (!xr_strcmp((*I).second.name(), level_name)) return ((*I).second);
...
IC void CGameGraph::set_current_level(const u32& level_id)
{
	...
	{
		if (level_id != vertex(i)->level_id()) continue;
