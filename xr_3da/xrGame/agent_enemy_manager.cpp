...
struct remove_wounded_predicate
{
	IC bool operator()(const CMemberEnemy& enemy) const
	{
		...
		if (!stalker->wounded()) return (false);
	...
	m_is_any_wounded = false;
	{
		...
		for (; I != E; ++I)
		{
			...
			else
			{
				...
				{
					const CAI_Stalker* stalker = smart_cast<const CAI_Stalker*>((*I).m_object);
					if (stalker && stalker->wounded()) m_is_any_wounded = true;
				}
			}
	...
	for (;;)
	{
		...
		for (; I != E; ++I)
		{
			...
			for (; J; J &= J - 1)
			{
				...
				if (!fsimilar((*i)->probability(), 1.f)) continue;
...
void CAgentEnemyManager::permutate_enemies()
{
	...
	for (; I != E; ++I)
	{
		...
		for (; i != e; ++i)
		{
			if ((*i).m_mask.is(member_mask)) (*I)->enemies().push_back(u32(i - b));
	...
	do
	{
		...
		for (; I != E; ++I)
		{
			...
			for (; i != e; ++i)
			{
				if ((*I)->selected_enemy() == *i) continue;
									   ...
									   const squad_mask_type& non_combat_members)
{
	...
	if (I != objects.end()) { (*I).m_squad_mask.assign((*I).m_squad_mask.get() | enemy.m_distribute_mask.get()); }
}
...
void CAgentEnemyManager::assign_wounded()
{
	...
	squad_mask_type assigned = 0;
	{
		...
		for (; I != E; ++I)
		{
			...
			if (!member_order) continue;

			squad_mask_type mask = object().member().mask((*I).second.first);
			if (!(object().member().combat_mask() & mask)) continue;
			...
			if (wounded_processor((*J).m_object) != ALife::_OBJECT_ID(-1)) continue;
	...
	while (population(assigned) < combat_member_count)
	{
		...
		for (int i = 0; i < 2; ++i)
		{
			...
			for (; I != E; ++I)
			{
				if (population((*I).m_distribute_mask.get()) > population_level) continue;
...
{
	if (!object().member().combat_mask()) return;
...
struct wounded_predicate
{
	...
	{
		if (wounded_enemy.first == m_object) return (true);

		if (wounded_enemy.second.first == m_object->ID()) return (true);
...
ALife::_OBJECT_ID CAgentEnemyManager::wounded_processor(const CEntityAlive* object)
{
	...
	for (; I != E; ++I)
	{
		if ((*I).first == object) return ((*I).second.first);
	}
...
bool CAgentEnemyManager::wounded_processed(const CEntityAlive* object) const
{
	...
	if (I == m_wounded.end()) return (false);
...
bool CAgentEnemyManager::assigned_wounded(const CEntityAlive* wounded, const CAI_Stalker* member)
{
	...
	for (; I != E; ++I)
	{
		if ((*I).m_object != wounded) continue;
...
{
	if (!object().member().registered_in_combat(member)) return (true);

	ENEMIES::const_iterator I = std::find(m_enemies.begin(), m_enemies.end(), enemy);
	if (I == m_enemies.end()) return (true);
