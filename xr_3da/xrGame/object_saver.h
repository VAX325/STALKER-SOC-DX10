...
struct CSaver
{
	...
	struct CHelper2<T, false>
	{
		IC static void save_data(const T& data, M& stream, const P& p)
		{
			constexpr bool is_base_and_derived =
				object_type_traits::is_base_and_derived_or_same_from_template<IPureSavableObject, T>::value;
			CHelper1<T, is_base_and_derived>::save_data(data, stream, p);
		}
	};
	...
	{
		IC static void save_data(const T& data, M& stream, const P& p) { CSaver<M, P>::save_data(*data, stream, p); }
	};
	...
	struct CHelper4<T, false>
	{
		IC static void save_data(const T& data, M& stream, const P& p)
		{
			...
			CHelper2<T, is_ptr>::save_data(data, stream, p);
		}
	};
	...
	{
		IC static void save_data(const T& data, M& stream, const P& p) { CHelper3::save_data(data, stream, p); }
	};
	...
	IC static void save_data(const T& data, M& stream, const P& p)
	{
		...
		CHelper4<T, is_stl>::save_data(data, stream, p);
	}
};
