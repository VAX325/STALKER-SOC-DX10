...
CInventory::CInventory()
{
	...
	for (u32 i = 0; i < m_slots.size(); ++i)
	{
		...
		if (pSettings->line_exist("inventory", temp)) m_slots[i].m_bPersistent = !!pSettings->r_bool("inventory", temp);
	};
...
void CInventory::Take(CGameObject* pObj, bool bNotActivate, bool strict_placement)
{
	...
	if (pIItem->m_pCurrentInventory)
	{
		...
		if (p) Msg("! object parent is [%s] [%d]", p->cName().c_str(), p->ID());
	}
	...
	if (!strict_placement) pIItem->m_eItemPlace = eItemPlaceUndefined;
	...
	switch (pIItem->m_eItemPlace)
	{
		case eItemPlaceBelt:
			result = Belt(pIItem);
#ifdef DEBUG
			if (!result) Msg("cant put in belt item %s", *pIItem->object().cName());
#endif
		...
		case eItemPlaceRuck:
			result = Ruck(pIItem);
#ifdef DEBUG
			if (!result) Msg("cant put in ruck item %s", *pIItem->object().cName());
#endif
...
#ifdef DEBUG
			if (!result) Msg("cant slot in ruck item %s", *pIItem->object().cName());
#endif
	...
	if (!CanPutInSlot(pIItem))
	{
		...
		Msg("there is item %s[%d,%x] in slot %d[%d,%x]", 
				*m_slots[pIItem->GetSlot()].m_pIItem->object().cName(), 
				...
				pIItem);
#endif
		if (m_slots[pIItem->GetSlot()].m_pIItem == pIItem && !bNotActivate) Activate(pIItem->GetSlot());
...
void CInventory::ActivateNextItemInActiveSlot()
{
	...
	for (; it != it_e; ++it)
	{
		...
		if (_pIItem->GetSlot() == m_iActiveSlot) new_item = _pIItem;

		if (b && new_item) break;
	}

	if (new_item == NULL) return; // only 1 item for this slot
...
{
	if (m_ActivationSlotReason == eKeyAction && reason == eImportUpdate) return false;
	...
	{
		if (m_slots[m_iActiveSlot].m_pIItem) m_slots[m_iActiveSlot].m_pIItem->Deactivate();
...
_finish:
	...
	if (res) m_ActivationSlotReason = reason;
	return res;
}
		...
		case kWPN_6:
			{
				...
				if (flags & CMD_START)
				{
					...
					{
						if ((int)m_iActiveSlot == cmd - kWPN_1 && !IsGameTypeSingle()) break;
		...
		case kARTEFACT:
			{
				if (flags & CMD_START)
				{
					...
					else { b_send_event = Activate(ARTEFACT_SLOT); }
				}
			}
			break;
	}

	if (b_send_event && g_pGameLevel && OnClient() && pActor) SendActionEvent(cmd, flags);
...
void CInventory::UpdateDropTasks()
{
	for (u32 i = 0; i < m_slots.size(); ++i)
	{
		if (m_slots[i].m_pIItem) UpdateDropItem(m_slots[i].m_pIItem);
	}
...
PIItem CInventory::Get(const char* name, bool bSearchRuck) const
{
	...
	{
		PIItem pIItem = *it;
		if (pIItem && !xr_strcmp(pIItem->object().cNameSect(), name) && pIItem->Useful()) return pIItem;
	}
	return NULL;
}
...
PIItem CInventory::Get(CLASS_ID cls_id, bool bSearchRuck) const
{
	...
	{
		PIItem pIItem = *it;
		if (pIItem && pIItem->object().CLS_ID == cls_id && pIItem->Useful()) return pIItem;
	}
	return NULL;
}
...
PIItem CInventory::Get(const u16 id, bool bSearchRuck) const
{
	...
	{
		PIItem pIItem = *it;
		if (pIItem && pIItem->object().ID() == id) return pIItem;
	}
	return NULL;
}
...
u32 CInventory::dwfGetSameItemCount(LPCSTR caSection, bool SearchAll)
{
	...
	for (TIItemContainer::iterator l_it = l_list.begin(); l_list.end() != l_it; ++l_it)
	{
		...
		if (l_pIItem && !xr_strcmp(l_pIItem->object().cNameSect(), caSection)) ++l_dwCount;
	}
...
bool CInventory::bfCheckForObject(ALife::_OBJECT_ID tObjectID)
{
	...
	{
		PIItem l_pIItem = *l_it;
		if (l_pIItem && l_pIItem->object().ID() == tObjectID) return (true);
	}
	return (false);
}
...
CInventoryItem* CInventory::get_object_by_id(ALife::_OBJECT_ID tObjectID)
{
	...
	{
		PIItem l_pIItem = *l_it;
		if (l_pIItem && l_pIItem->object().ID() == tObjectID) return (l_pIItem);
	}
	return (0);
}
...
{
	if (pIItem->GetSlot() < m_slots.size() && m_slots[pIItem->GetSlot()].m_pIItem == pIItem) return true;
	return false;
}
...
bool CInventory::CanPutInSlot(PIItem pIItem) const
{
	...
	if (pIItem->GetSlot() < m_slots.size() && m_slots[pIItem->GetSlot()].m_pIItem == NULL) return true;
...
bool CInventory::CanTakeItem(CInventoryItem* inventory_item) const
{
	...
	if (!pActor && (TotalWeight() + inventory_item->Weight() > m_pOwner->MaxCarryWeight())) return false;

	return true;
}

u32 CInventory::BeltWidth() const { return m_iMaxBelt; }
...
void CInventory::AddAvailableItems(TIItemContainer& items_container, bool for_trade) const
{
	...
	{
		PIItem pIItem = *it;
		if (!for_trade || pIItem->CanTrade()) items_container.push_back(pIItem);
	}
	...
	if (m_bSlotsUseful)
	{
		...
		for (; slot_it != slot_it_e; ++slot_it)
		{
			...
			{
				if (!S.m_bPersistent || S.m_pIItem->GetSlot() == GRENADE_SLOT) items_container.push_back(S.m_pIItem);
			}
		}
	}
}
...
bool CInventory::isBeautifulForActiveSlot(CInventoryItem* pIItem)
{
	...
	{
		if ((*it).m_pIItem && (*it).m_pIItem->IsNecessaryItem(pIItem)) return (true);
	}
	return (false);
}
...
void CInventory::SetSlotsBlocked(u16 mask, bool bBlock)
{
	...
	for (int i = 0; i < SLOTS_TOTAL; ++i)
	{
		if (mask & (1 << i))
		{
			...
			if (bCanBeActivated != m_slots[i].CanBeActivated()) bChanged = true;
		}
	}
	if (bChanged)
	{
		...
		{ // try to restore hidden weapon
			if (PrevActiveSlot != NO_ACTIVE_SLOT && m_slots[PrevActiveSlot].CanBeActivated())
				if (Activate(PrevActiveSlot)) SetPrevActiveSlot(NO_ACTIVE_SLOT);
		}
		else
		{ // try to hide active weapon
			if (!m_slots[ActiveSlot].CanBeActivated())
				if (Activate(NO_ACTIVE_SLOT)) SetPrevActiveSlot(ActiveSlot);
		}
	}
}
