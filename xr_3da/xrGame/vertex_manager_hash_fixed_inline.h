...
IC bool CHashFixedVertexManager::is_visited(const _index_type& vertex_id) const
{
	...
	if (!vertex || (vertex->m_path_id != current_path_id()) || (vertex->m_hash != index)) return (false);
	for (; vertex; vertex = vertex->m_next)
		if (vertex->m_vertex->index() == vertex_id) return (true);
	return (false);
}
...
IC typename CHashFixedVertexManager::CGraphVertex& CHashFixedVertexManager::get_node(const _index_type& vertex_id) const
{
	...
	for (; vertex; vertex = vertex->m_next)
		if (vertex->m_vertex->index() == vertex_id) return (*vertex->m_vertex);
	NODEFAULT;
#ifdef DEBUG
																						  ...
																						  const _index_type& vertex_id)
{
	...
	if (index_vertex->m_prev)
	{
		...
		if (index_vertex->m_next) index_vertex->m_next->m_prev = index_vertex->m_prev;
	}
	else
	{
		if (index_vertex->m_next) index_vertex->m_next->m_prev = 0;
	...
	if (!_vertex || (_vertex->m_path_id != current_path_id()) || (_vertex->m_hash != index)) _vertex = 0;

	m_hash[index] = index_vertex;
	index_vertex->m_next = _vertex;
	index_vertex->m_prev = 0;
	if (_vertex) _vertex->m_prev = index_vertex;
