...
struct CDataStorageDoubleLinkedList
{
	...
	using _d_vertex = typename DoubleLinkedList<T1>::_vertex<T2>;

	template <typename _data_storage, template <typename _T> class _vertex = CEmptyClassTemplate>
	struct smart_inherated
	{
		template <typename _T>
		using VertexTemplate = _d_vertex<_vertex, _T>;

		using inheratable =
			typename CDataStorageSingleLinkedList<sorted>::template CDataStorage<_data_storage, VertexTemplate>;
	};
