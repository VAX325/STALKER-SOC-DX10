...
void CMainMenu::Activate(bool bActivate)
{
	...
	if (bActivate)
	{
		...
		if (g_pGameLevel)
		{
			if (b_is_single) { Device.seqFrame.Remove(g_pGameLevel); }
	...
	else
	{
		...
		if (g_pGameLevel)
		{
			if (b_is_single) { Device.seqFrame.Add(g_pGameLevel); }
...
void CMainMenu::IR_OnMouseMove(int x, int y)
{
	...
	if (MainInputReceiver()) MainInputReceiver()->IR_OnMouseMove(x, y);
};
...
void CMainMenu::IR_OnKeyboardHold(int dik)
{
	...
	if (MainInputReceiver()) MainInputReceiver()->IR_OnKeyboardHold(dik);
};
...
void CMainMenu::OnRender()
{
	...
	if (g_pGameLevel) Render->Calculate();
...
void CMainMenu::OnFrame()
{
	...
	if (IsActive() || m_sPDProgress.IsInProgress) m_pGameSpyFull->Update();

	if (IsActive()) CheckForErrorDlg();
}
...
{
	if (mode != IRender_interface::SM_FOR_GAMESAVE) { ::Render->Screenshot(mode, name); }
	else
	{
...
void CMainMenu::OnNoNewPatchFound() { StartStopMenu(m_pMB_ErrDlgs[NoNewPatch], false); }
...
LPCSTR CMainMenu::GetGSVer()
{
	...
	if (m_pGameSpyFull) { strcpy(buff2, m_pGameSpyFull->GetGameVersion(buff)); }
	else
	{
		buff[0] = 0;
		buff2[0] = 0;
