...
void game_PlayerState::net_Import(NET_Packet& P)
{
	...
	if (bFullUpdate) P.r_stringZ(name);
...
{
	if (mOldIDs.size() >= 10) { mOldIDs.pop_front(); };
...
CLASS_ID game_GameState::getCLASS_ID(LPCSTR game_type_name, bool isServer)
{
	if (!g_dedicated_server)
	{
		...
		const char* tmp_clsid = result(game_type_name, isServer);
		shared_str clsid = tmp_clsid;

		xr_delete(l_tpIniFile);
		if (clsid.size() == 0) Debug.fatal(DEBUG_INFO, "Unknown game type: %s", game_type_name);
	...
	if (isServer)
	{
		if (!xr_strcmp(game_type_name, "single")) return (TEXT2CLSID("SV_SINGL"));

		if (!xr_strcmp(game_type_name, "deathmatch")) return (TEXT2CLSID("SV_DM"));

		if (!xr_strcmp(game_type_name, "teamdeathmatch")) return (TEXT2CLSID("SV_TDM"));

		if (!xr_strcmp(game_type_name, "artefacthunt")) return (TEXT2CLSID("SV_AHUNT"));

		return (TEXT2CLSID(""));
	}

	if (!xr_strcmp(game_type_name, "single")) return (TEXT2CLSID("CL_SINGL"));

	if (!xr_strcmp(game_type_name, "deathmatch")) return (TEXT2CLSID("CL_DM"));

	if (!xr_strcmp(game_type_name, "teamdeathmatch")) return (TEXT2CLSID("CL_TDM"));

	if (!xr_strcmp(game_type_name, "artefacthunt")) return (TEXT2CLSID("CL_AHUNT"));
