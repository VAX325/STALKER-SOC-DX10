...
{
	m_use_log = value;
	typename CPlanner::OPERATOR_VECTOR::iterator I = m_operators.begin();
	typename CPlanner::OPERATOR_VECTOR::iterator E = m_operators.end();
	for (; I != E; ++I)
...
IC void CPlanner::show_current_world_state()
{
	...
	typename CPlanner::EVALUATORS::const_iterator I = evaluators().begin();
	typename CPlanner::EVALUATORS::const_iterator E = evaluators().end();
	for (; I != E; ++I)
	{
...
IC void CPlanner::show_target_world_state()
{
	...
	for (; I != E; ++I)
	{
		typename xr_vector<COperatorCondition>::const_iterator J = std::lower_bound(
...
IC void CPlanner::show(LPCSTR offset)
{
	...
	{
		Msg("\n%sOPERATORS : %d\n", offset, CPlanner::operators().size());
		typename CPlanner::OPERATOR_VECTOR::const_iterator I = CPlanner::operators().begin();
		typename CPlanner::OPERATOR_VECTOR::const_iterator E = CPlanner::operators().end();
		for (; I != E; ++I)
		{
...
{
	{
		typename CPlanner::EVALUATORS::iterator I = m_evaluators.begin();
		typename CPlanner::EVALUATORS::iterator E = m_evaluators.end();
		for (; I != E; ++I)
