...
IC u32 CSoundMemoryManager::priority(const MemorySpace::CSoundObject& sound) const
{
	...
	for (; I != E; ++I)
		if (((*I).second < priority) && ((*I).first & sound.m_sound_type) == (*I).first) priority = (*I).second;
	return (priority);
}
...
{
#ifndef MASTER_GOLD
	if (object && (object->CLS_ID == CLSID_OBJECT_ACTOR) && psAI_Flags.test(aiIgnoreActor)) return;
#endif // MASTER_GOLD

	VERIFY(_valid(sound_power));
	if (!m_sounds) return;

	if (user_data) user_data->accept(m_visitor);
	...
	if (!entity_alive->g_Alive()) return;

	VERIFY(_valid(sound_power));
	if (is_sound_type(sound_type, SOUND_TYPE_WEAPON)) sound_power *= m_weapon_factor;

	VERIFY(_valid(sound_power));
	if (is_sound_type(sound_type, SOUND_TYPE_ITEM)) sound_power *= m_item_factor;

	VERIFY(_valid(sound_power));
	if (is_sound_type(sound_type, SOUND_TYPE_MONSTER)) sound_power *= m_npc_factor;

	VERIFY(_valid(sound_power));
	if (is_sound_type(sound_type, SOUND_TYPE_ANOMALY)) sound_power *= m_anomaly_factor;

	VERIFY(_valid(sound_power));
	if (is_sound_type(sound_type, SOUND_TYPE_WORLD)) sound_power *= m_world_factor;
...
void CSoundMemoryManager::add(const CSoundObject& sound_object, bool check_for_existance)
{
	...
	{
		if (m_sounds->end() != std::find(m_sounds->begin(), m_sounds->end(), object_id(sound_object.m_object))) return;
	}
...
void CSoundMemoryManager::add(const CObject* object, int sound_type, const Fvector& position, float sound_power)
{
	...
	if (object && (m_object->ID() == object->ID())) return;
#endif

#ifndef SAVE_OWN_ITEM_SOUNDS
	// we do not want to save the sounds which was from the items we own
	if (object && object->H_Parent() && (object->H_Parent()->ID() == m_object->ID())) return;
#endif

#ifndef SAVE_NON_ALIVE_OBJECT_SOUNDS
	// we do not want to save sounds from the non-alive objects (?!)
	if (object && !m_object->memory().enemy().selected() && !smart_cast<const CEntityAlive*>(object)) return;
#endif
	...
	if (entity_alive && me && (me->tfGetRelationType(entity_alive) == ALife::eRelationTypeFriend)) return;
#endif
	...
	if (m_object->memory().visual().visible_now(entity_alive)) return;
#endif

	const CGameObject* game_object = smart_cast<const CGameObject*>(object);
	if (!game_object && object) return;
	...
	if (m_sounds->end() == J)
	{
		...
		if (!game_object) sound_object.m_object_params.m_position = position;
	...
	else
	{
		...
		if (!game_object) (*J).m_object_params.m_position = position;
	}
}
...
struct CRemoveOfflinePredicate
{
	...
	{
		if (!object.m_object) return (false);
...
struct CSoundObjectPredicate
{
	...
	{
		if (!m_object) return (!sound_object.m_object);

		if (!sound_object.m_object) return (false);
...
void CSoundMemoryManager::remove_links(CObject* object)
{
	...
	if (I != m_sounds->end()) m_sounds->erase(I);

#ifdef USE_SELECTED_SOUND
	if (!m_selected_sound) return;

	if (!m_selected_sound->m_object) return;

	if (m_selected_sound->m_object->ID() != object->ID()) return;
