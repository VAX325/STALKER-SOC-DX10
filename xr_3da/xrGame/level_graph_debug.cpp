...
{
	if (m_current_level_id == level_id) return;
...
void CLevelGraph::render()
{
	...
	if (!bDebug && !psAI_Flags.test(aiMotion)) return;

	if (bDebug && psAI_Flags.test(aiDebug)) draw_nodes();

	draw_restrictions();

	if (psAI_Flags.test(aiCover)) draw_covers();

	if (!psHUD_Flags.test(HUD_DRAW)) return;

	if (psAI_Flags.test(aiMotion)) draw_objects();
...
void CLevelGraph::draw_vertex(const int& vertex_id)
{
	...
	for (; I != E; ++I)
	{
		...
		if (neighbour_id < vertex_id) draw_edge(vertex_id, neighbour_id);
	}
}
...
void CLevelGraph::draw_stalkers(const int& vertex_id)
{
	...
	if (show_text)
	{
		...
		for (; I != E; ++I)
		{
			...
			if ((path.size() >= 2) && !fis_zero(walked_distance)) continue;
	...
	for (; I != E; ++I)
	{
		...
		if (fis_zero(walked_distance)) continue;
		...
		if (temp.z < 0.f) continue;

		if (temp.w < 0.f) continue;

		if (temp.x < -1.f) continue;

		if (temp.x > 1.f) continue;

		if (temp.y < -1.f) continue;
...
void CLevelGraph::draw_game_graph()
{
	...
	for (int i = 0, n = (int)graph.header().vertex_count(); i < n; ++i)
	{
		...
		if (psAI_Flags.test(aiDrawGameGraphStalkers)) draw_stalkers(i);

		if (psAI_Flags.test(aiDrawGameGraphObjects)) draw_objects(i);
	}
