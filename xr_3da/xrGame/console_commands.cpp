...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		Msg("* [x-ray]: process heap[%d K], game lua[%d K], render[%d K]", _process_heap / 1024, _game_lua / 1024,
			_render / 1024);
#endif // SEVERAL_ALLOCATORS
...
#endif
		if (0 == g_pGameLevel) { Msg("! There are no level(s) started"); }
		else
		{
			Console->Hide();
			string_path fn;
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		if ((*value_blin > 0) && g_pGameLevel) { g_dwInputUpdateDelta = 1000 / (*value_blin); };
	}
};
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		else if (strstr(cName, "script_debug_stop") == cName) { ai().script_engine().stopDebugger(); }
		else if (strstr(cName, "script_debug_restart") == cName) { ai().script_engine().restartDebugger(); };
	};
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		bool bWhatToDo = TRUE;
		if (0 == xr_strlen(args)) { bWhatToDo = !MainMenu()->IsActive(); };
