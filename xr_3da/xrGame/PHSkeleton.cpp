...
bool CPHSkeleton::Spawn(CSE_Abstract* D)
{
	...
	else
	{
		...
		if (K)
		{
			...
			{
				if (ini->line_exist("collide_parts", "small_object")) { obj->PPhysicsShell()->SetSmall(); }
...
void CPHSkeleton::RestoreNetState(CSE_PHSkeleton* po)
{
	...
	if (obj->PPhysicsShell() && obj->PPhysicsShell()->isActive()) { obj->PPhysicsShell()->Disable(); }
...
void CPHSkeleton::UnsplitSingle(CPHSkeleton* SO)
{
	...
	mask0.invert();
	mask1.m_and(mask0.flags); // second part mask
