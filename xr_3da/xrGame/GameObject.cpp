...
void CGameObject::reinit()
{
	...
	if (!g_dedicated_server) ai_location().reinit();
...
{
#ifdef DEBUG
	if (psAI_Flags.test(aiDestroy)) Msg("Destroying client object [%d][%s][%x]", ID(), *cName(), this);
#endif

	VERIFY(m_spawned);
	if (animation_movement_controlled()) destroy_anim_mov_ctrl();

	xr_delete(m_ini_file);

	m_script_clsid = -1;
	if (Visual() && smart_cast<IKinematics*>(Visual())) smart_cast<IKinematics*>(Visual())->Callback(0, 0);
		...
		case GE_HIT_STATISTIC:
			{
				...
				*/
				...
				switch (HDS.PACKET_TYPE)
				{
					case GE_HIT_STATISTIC:
						{
							if (GameID() != GAME_SINGLE) Game().m_WeaponUsageStatistic->OnBullet_Check_Request(&HDS);
						}
						break;
					default:
						{
				...
				Hit(&HDS);
				//---------------------------------------------------------------------------
				if (GameID() != GAME_SINGLE) Game().m_WeaponUsageStatistic->OnBullet_Check_Result(false);
				//---------------------------------------------------------------------------
			}
			break;
		case GE_DESTROY:
			{
				if (H_Parent())
				{
					Msg("GE_DESTROY arrived, but H_Parent() exist. object[%d][%s] parent[%d][%s] [%d]", ID(),
						cName().c_str(), H_Parent()->ID(), H_Parent()->cName().c_str(), Device.dwFrame);
				}
				setDestroy(TRUE);
			}
			break;
...
BOOL CGameObject::net_Spawn(CSE_Abstract* DC)
{
	...
	if (E->name_replace()[0]) cName_set(E->name_replace());
...
#pragma warning(disable : 4238)
		auto reader = IReader((void*)(*(O->m_ini_string)), O->m_ini_string.size());
...
#pragma warning(pop)
	...
	reload(*cNameSect());
	if (!g_dedicated_server) CScriptBinder::reload(*cNameSect());

	reinit();
	if (!g_dedicated_server) CScriptBinder::reinit();
#ifdef DEBUG
	if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && stricmp(PH_DBG_ObjectTrackName(), *cName()) == 0)
	{
...
void CGameObject::net_Save(NET_Packet& net_packet)
{
	...
	if (psAI_Flags.test(aiSerialize)) { Msg(">> After save :: packet position = [%u]", net_packet.w_tell()); }
#endif
...
void CGameObject::net_Load(IReader& ireader)
{
	...
	if (psAI_Flags.test(aiSerialize)) { Msg(">> After load :: reader position = [%i]", ireader.tell()); }
#endif
	// ----------------------------------------------------------
#ifdef DEBUG
	if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && stricmp(PH_DBG_ObjectTrackName(), *cName()) == 0)
...
{
	if (!spawn_ini() || ai().get_alife()) return;

	if (!spawn_ini()->section_exist("spawn")) return;
	...
	for (u32 k = 0, j; spawn_ini()->r_line("spawn", k, &N, &V); k++)
	{
		...
		if (V && xr_strlen(V))
		{
			...
			if (fis_zero(p)) p = 1.f;
			if (!j) j = 1;
			if (NULL != strstr(V, "cond=")) f_cond = (float)atof(strstr(V, "cond=") + 5);
		...
		for (u32 i = 0; i < j; ++i)
			if (::Random.randF(1.f) < p)
			{
				...
				if (pSE_InventoryItem) pSE_InventoryItem->m_fCondition = f_cond;
...
void CGameObject::setup_parent_ai_locations(bool assign_position)
{
	...
	if (assign_position && use_parent_ai_locations()) Position().set(l_tpGameObject->Position());

	// setup its ai locations
	if (!UsedAI_Locations()) return;
...
// seconds)",Device.dwTimeGlobal,*cName(),timer.GetElapsed_sec());
	...
	if (decrement_reference && (ai_location().level_vertex_id() == l_dwNewLevelVertexID)) return;
...
{
#ifdef DEBUG
	if (!m_spawned) Msg("! you are trying to use a destroyed object [%x]", this);
#endif
	THROW(m_spawned);
	if (!m_lua_game_object) m_lua_game_object = xr_new<CScriptGameObject>(const_cast<CGameObject*>(this));
...
void CGameObject::shedule_Update(u32 dt)
{
	...
	if (!g_dedicated_server) CScriptBinder::shedule_Update(dt);
}
...
void CGameObject::net_Relcase(CObject* O)
{
	...
	if (!g_dedicated_server) CScriptBinder::net_Relcase(O);
}
...
void CGameObject::update_animation_movement_controller()
{
	...
	if (m_anim_mov_ctrl->isActive()) return;
