...
namespace SmartDynamicCast
{
	...
	struct conversion_sequence
	{
		...
		struct list_iterator
		{
			...
			typedef
				typename selector<can_use_heritage &&
...
#pragma todo("Dima to all : this smart_cast is not optimized!")
	...
	template <typename T1, typename T2, bool base>
	struct CHelper1;

	template <typename T1, typename T2>
	struct CHelper1<T1, T2, false>
	{
		IC static T1* smart_cast(T2* p)
		{
			return (CSmartMatcher<T1, T2>::smart_cast<typename get_conversion_sequence<T1, T2>::result>(p));
		}
	};

	template <typename T1, typename T2>
	struct CHelper1<T1, T2, true>
	{
		IC static T1* smart_cast(T2* p) { return (static_cast<T1*>(p)); }
	};
	...
	IC T1* smart_cast(T2* p)
	{
		constexpr bool checker =
			object_type_traits::is_base_and_derived<T1, T2>::value || object_type_traits::is_same<T1, T2>::value;
		return (CHelper1<T1, T2, checker>::smart_cast(p));
	}
	...
	struct CHelper2
	{
		...
		IC static T1* smart_cast(T2* p)
		{
			...
			VERIFY2(temp == test, make_string("SmartCast<%s*>(%s*) FAILED (result differs from the dynamic_cast) or "
...
#endif
	return (*SmartDynamicCast::CHelper2<T2>::smart_cast<typename object_type_traits::remove_reference<T1>::type>(&p));
}
