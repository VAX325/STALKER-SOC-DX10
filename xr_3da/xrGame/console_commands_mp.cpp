...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		if (it == cnt) { Msg("! No such player found : %s", PlayerName); }
...
public:
	...
	virtual void Execute(LPCSTR args) { Game().m_WeaponUsageStatistic->SaveData(); }
...
public:
	...
	bool ExecuteGetState(LPCSTR args)
	{
		if (args[0] == 0)
		{
			if (Level().battleye_system.server) { Msg("BattlEye Server is enabled"); }
			else { Msg("BattlEye Server is disabled"); }
			return true;
		}
		return false;
	}
	...
	virtual void Info(TInfo& I) { strcpy(I, "BattlEye Server commands"); }
};
...
public:
	...
	bool ExecuteGetState(LPCSTR args)
	{
		if (args[0] == 0)
		{
			if (Level().battleye_system.client) { Msg("BattlEye Client is enabled"); }
			else { Msg("BattlEye Client is disabled"); }
			return true;
		}
		return false;
	}
	...
	virtual void Info(TInfo& I) { strcpy(I, "BattlEye Client commands"); }
};
...
public:
	...
	{
		if (!OnServer()) return;
		if (Level().Server && Level().Server->game) { Console->Execute("cfg_load all_server_settings"); }
	}
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		if (Level().Server && Level().Server->game)
		{
			...
			if (game) game->SvSendChatMessage(args);
		}
	}
};
...
public:
	...
	{
		if (!OnServer()) return;
		if (Level().Server && Level().Server->game) { Level().Server->game->DumpOnlineStatistic(); }
	}
	virtual void Info(TInfo& I) { strcpy(I, "Shows current server settings"); }
};
