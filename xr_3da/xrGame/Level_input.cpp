...
void CLevel::IR_OnKeyboardPress(int key)
{
	...
	switch (_curr)
	{
		...
		case kPAUSE:
			if (!g_block_pause)
			{
				if (IsGameTypeSingle()) { Device.Pause(!Device.Paused(), TRUE, TRUE, "li_pause_key"); }
			}
			return;
			break;
	};
	...
	switch (key)
	{
		...
		case DIK_F4:
			{
				if (pInput->iGetAsyncKeyState(DIK_LALT)) break;

				if (pInput->iGetAsyncKeyState(DIK_RALT)) break;

				bool bOk = false;
				u32 i = 0, j, n = Objects.o_count();
				if (pCurrentEntity)
					for (; i < n; ++i)
						if (Objects.o_get_by_iterator(i) == pCurrentEntity) break;
				if (i < n)
				{
					j = i;
					bOk = false;
...
//		return;
	...
	if (obj)
	{
		...
		if (monster) monster->debug_on_key(key);
	}
#endif
}
