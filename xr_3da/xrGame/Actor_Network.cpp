...
{
	VERIFY(g_actor);
	if (GameID() != GAME_SINGLE) VERIFY(g_actor == Level().CurrentControlEntity());
	return (g_actor);
};
...
void CActor::net_Export(NET_Packet& P) // export to server
{
	...
	if (H_Parent() || (GameID() == GAME_SINGLE) || ((NumItems > 1) && OnClient())) NumItems = 0;
...
void CActor::net_Import_Base(NET_Packet& P)
{
	...
	//------------------------------------------------
	{
		...
		{
			if (inventory().GetActiveSlot() != u32(ActiveSlot)) inventory().Activate(u32(ActiveSlot));
		};
	}
	...
	if (!NET.empty() && N.dwTimeStamp == NET.back().dwTimeStamp) { NET.back() = N; }
	else
	{
		NET.push_back(N);
...
void CActor::net_Import_Physic(NET_Packet& P)
{
	...
	else
	{
		...
		//		if (g_Alive() && (Remote() || OnServer()))
		{
			...
			if (!NET_A.empty() && N_A.dwTimeStamp == NET_A.back().dwTimeStamp) { NET_A.back() = N_A; }
			else
			{
				NET_A.push_back(N_A);
...
BOOL CActor::net_Spawn(CSE_Abstract* DC)
{
	...
	if (OnServer()) { E->s_flags.set(M_SPAWN_OBJECT_LOCAL, TRUE); }

	if (TRUE == E->s_flags.test(M_SPAWN_OBJECT_LOCAL) && TRUE == E->s_flags.is(M_SPAWN_OBJECT_ASPLAYER)) g_actor = this;
	...
	if (E->m_holderID != u16(-1)) { character_physics_support()->movement()->DestroyCharacter(); }
	...
	if (!IsGameTypeSingle()) { setEnabled(TRUE); }
	...
	*/
	...
	if (E->m_holderID != ALife::_OBJECT_ID(-1))
		if (!g_dedicated_server) Level().client_spawn_manager().add(E->m_holderID, ID(), callback);
	// F
	//-------------------------------------------------------------
	...
	if (Level().IsDemoPlay() && OnClient()) { setLocal(FALSE); };
	return TRUE;
}
...
void CActor::net_Destroy()
{
	...
	if (m_holder_id != ALife::_OBJECT_ID(-1))
		if (!g_dedicated_server) Level().client_spawn_manager().remove(m_holder_id, ID());

	delete_data(m_game_task_manager);
	delete_data(m_statistic_manager);

	if (!g_dedicated_server) Level().MapManager().RemoveMapLocationByObjectID(ID());
...
void CActor::net_Relcase(CObject* O)
{
	...
	if (GO && m_pObjectWeLookingAt == GO) { m_pObjectWeLookingAt = NULL; }
	CHolderCustom* HC = smart_cast<CHolderCustom*>(GO);
	if (HC && HC == m_pVehicleWeLookingAt) { m_pVehicleWeLookingAt = NULL; }
	if (HC && HC == m_holder)
	{
	...
	if (!g_dedicated_server) memory().remove_links(O);
	m_pPhysics_support->in_NetRelcase(O);
}

BOOL CActor::net_Relevant() // relevant for export to server
{
	if (OnServer()) { return getSVU() | getLocal(); }
	else { return Local() & g_Alive(); };
};
...
void CActor::PH_B_CrPr() // actions & operations before physic correction-prediction steps
{
	...
	if (g_Alive())
	{
		CrPr_SetActivated(true);
		{
			...
			if (pIStart->o_torso.roll > PI) pIStart->o_torso.roll -= PI_MUL_2;
		}
		///////////////////////////////////////////////
		CPHSynchronize* pSyncObj = NULL;
		pSyncObj = PHGetSyncItem(0);
		...
		else
		{
			...
			if (!N_A.State.enabled) { pSyncObj->set_State(N_A.State); }
			else
			{
				PHUnFreeze();
...
void CActor::CalculateInterpolationParams()
{
	...
	*/
	...
	else
	{
		...
		else { HP1.sub(LastState.position, LastState.previous_position); };
...
void CActor::OnRender_Network()
{
	...
	if (g_Alive())
	{
		...
		for (int i = 0; i < 11; i++)
		{
			...
			if (i != 0) { Level().debug_renderer().draw_line(*pM, *ppoint0, *ppoint1, cColor); };
			point0S.set(point1S);
			point0H.set(point1H);
			point0L.set(point1L);
		};
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (g_cl_InterpolationType > 0) { Level().debug_renderer().draw_line(*pM, *ppoint0, *ppoint1, sColor); }
		}
	...
	for (u32 o_it = 0; o_it < ISpatialResult.size(); o_it++)
	{
		...
		if (APU_Box.Pick(pIItem->object().Position(), pIItem->object().Position()))
		{
			if (GameID() == GAME_DEATHMATCH || GameID() == GAME_TEAMDEATHMATCH)
			{
				...
				{
					if (inventory().ItemFromSlot(pIItem->GetSlot())) { continue; }
				}
			}
			NET_Packet P;
...
void CActor::OnCriticalHitHealthLoss()
{
	...
	{
		if (pLastHittingWeapon->CLS_ID == CLSID_OBJECT_W_KNIFE) SpecialHit = SKT_KNIFEKILL;
	}
	...
	u_EventSend(P);
	//-------------------------------------------
	if (GameID() != GAME_SINGLE) Game().m_WeaponUsageStatistic->OnBullet_Check_Result(true);
};
...
bool CActor::InventoryAllowSprint()
{
	...
	if (pActiveItem && !pActiveItem->IsSprintAllowed()) { return false; };
	PIItem pOutfitItem = inventory().ItemFromSlot(OUTFIT_SLOT);
	if (pOutfitItem && !pOutfitItem->IsSprintAllowed()) { return false; }
	return true;
};
