...
bool CController::can_tube_fire()
{
	if (m_tube_at_once)
	{
		if (EnemyMan.get_enemy() && EnemyMan.see_enemy_now() && m_psy_hit->check_start_conditions()) return true;
...
void CController::debug_on_key(int key)
{
	switch (key)
	{
		case DIK_MINUS:
			...
			if (m_psy_hit->check_start_conditions()) { control().activate(ControlCom::eComCustom1); }
