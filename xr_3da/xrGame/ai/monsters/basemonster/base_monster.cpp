...
void CBaseMonster::Hit(SHit* pHDS)
{
	...
	if (g_Alive())
		if (!critically_wounded()) update_critical_wounded(pHDS->boneID, pHDS->power);
...
{
	auto hory_sheeet = SHit(P, dir, who, element, p_in_object_space, impulse, hit_type);
	m_pPhysics_support->in_Hit(hory_sheeet);
}
...
{
	if (!movement().restrictions().accessible(object->Position())) return (false);

	if (!movement().restrictions().accessible(object->ai_location().level_vertex_id())) return (false);
		...
		else
		{
			...
			switch (type)
			{
				case MonsterSound::eMonsterSoundIdle:
					...
					else { delay = u32(float(db().m_dwIdleSndDelay) * _sqrt(float(objects_count))); }
