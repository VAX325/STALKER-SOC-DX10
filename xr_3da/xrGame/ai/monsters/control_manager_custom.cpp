										   ...
										   u32 vel_mask_ground, u32 flags)
{
	...
	if (s2)
	{
		...
		m_jump->setup_data().flags.m_or(SControlJumpData::ePrepareInMove);
	}
	else
	...
	else
	{
		...
		m_jump->setup_data().flags.m_or(SControlJumpData::eGroundSkip);
	}

	if (!s1 && !s2) { m_jump->setup_data().flags.m_or(SControlJumpData::ePrepareSkip); }

	m_jump->setup_data().flags.m_or(SControlJumpData::eGlidePlayAnimOnce);
	m_jump->setup_data().flags.m_or(SControlJumpData::eGlideOnPrepareFailed);
...
void CControlManagerCustom::jump(const Fvector& position)
{
	...
	ctrl_data->flags.m_or(SControlJumpData::ePrepareSkip);
	ctrl_data->force_factor = -1.f;
											   ...
											   LPCSTR right1, LPCSTR right2, float angle, u32 flags)
{
	...
	else { data.anim_stop_ls.invalidate(); }
	...
	else { data.anim_run_ls.invalidate(); }
	...
	else { data.anim_stop_rs.invalidate(); }
	...
	else { data.anim_run_rs.invalidate(); }
}
