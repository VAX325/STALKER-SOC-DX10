...
void CMonsterEnemyManager::update()
{
	...
	if ((prev_enemy == enemy) && (time_last_seen != Device.dwTimeGlobal)) flags.m_or(FLAG_ENEMY_LOST_SIGHT);
	if (prev_enemy && !prev_enemy->g_Alive()) flags.m_or(FLAG_ENEMY_DIE);
	if (!enemy_see_me) flags.m_or(FLAG_ENEMY_DOESNT_SEE_ME);
	...
	if (prev_enemy == enemy)
	{
		...
		if (_abs(dist_now - dist_prev) < 0.2f)
			flags.m_or(FLAG_ENEMY_STANDING);
		else
		{
			if (dist_now < dist_prev)
				flags.m_or(FLAG_ENEMY_GO_CLOSER);
			else
				flags.m_or(FLAG_ENEMY_GO_FARTHER);

			if (_abs(dist_now - dist_prev) < 1.2f)
			{
				if (dist_now < dist_prev)
					flags.m_or(FLAG_ENEMY_GO_CLOSER_FAST);
				else
					flags.m_or(FLAG_ENEMY_GO_FARTHER_FAST);
			}
		}

		if (flags.is(FLAG_ENEMY_STANDING) && flags.is(FLAG_ENEMY_DOESNT_SEE_ME))
			flags.m_or(FLAG_ENEMY_DOESNT_KNOW_ABOUT_ME);
	}
	else
		flags.m_or(FLAG_ENEMY_STATS_NOT_READY);
...
{
	if (Actor() == enemy) { return (Actor()->memory().visual().visible_right_now(monster)); }
	else
	{
...
{
	if (object0->Position().distance_to(object1->Position()) > object0->ffGetRange()) return (false);
	...
	if ((angle_difference(yaw1, yaw2) <= fYawFov) && (angle_difference(pitch1, pitch2) <= fPitchFov)) return (true);
	return (false);
}
