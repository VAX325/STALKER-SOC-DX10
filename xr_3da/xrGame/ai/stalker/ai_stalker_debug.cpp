...
void draw_planner(const planner_type& brain, LPCSTR start_indent, LPCSTR indent, LPCSTR planner_id)
{
	...
	if (planner)
	{
		LPCSTR planer_id = _brain.action2string(brain.solution().front());
		draw_planner<CScriptActionPlannerAction>(*planner, start_indent, indent, planer_id);
	}
	...
	for (; I != E; ++I)
	{
		typename xr_vector<planner_type::COperatorCondition>::const_iterator J =
