			...
			else
			{
				if (!already_critically_wounded && became_critically_wounded)
				{
					if (HDS.who)
					{
						...
						if (stalker) stalker->on_critical_wound_initiator(this);
					}
				}
			}
		}
...
void CAI_Stalker::HitSignal(float amount, Fvector& vLocalDir, CObject* who, s16 element)
{
	...
	if (g_Alive()) memory().hit().add(amount, vLocalDir, who, element);
}
...
bool CAI_Stalker::fire_make_sense()
{
	...
	if (best_weapon()->object().ef_weapon_type() != 6) return (false);
...
void CAI_Stalker::wounded(bool value)
{
	...
	if (!m_wounded && g_Alive()) character_physics_support()->CreateCharacter();
...
static float throw_pick_error(float low, float high, const Fvector& position, const Fvector& velocity,
...
static float throw_select_pick_time(const float& start_low, float high, const Fvector& start, const Fvector& velocity,
									const Fvector& gravity, const float& epsilon)
{
	float low = start_low;
...
bool CAI_Stalker::throw_check_error(float low, float high, const Fvector& position, const Fvector& velocity,
									const Fvector& gravity)
{
	Fvector start;
	throw_position(start, position, velocity, gravity, low);
	...
	if (m_throw_ignore_object) m_throw_ignore_object->setEnabled(throw_ignore_object_enabled);

	setEnabled(previous_enabled);

	if (range < distance) m_throw_collide_position.mad(start, start_to_target, range);
...
void CAI_Stalker::check_throw_trajectory(const float& throw_time)
{
	...
	for (;;)
	{
		...
		{
			if (fsimilar(time, high) && m_throw_collide_position.similar(m_throw_target, .2f)) break;
