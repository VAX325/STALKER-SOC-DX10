											  ...
											  const CSConditionState& self_condition) const
{
	...
	for (; (I != E) && (i != e);)
		...
		else if ((*I).condition() > (*i).condition())
		{
			...
			{
				if ((*J).value() != (*i).value()) return (false);
				++J;
			}
			++i;
		}
																				...
																				T& problem_solver) const
{
	...
	for (; (I != E) && (i != e);)
	{
		...
		else if ((*I).condition() > (*i).condition())
		{
			...
			if ((II == EE) || ((*II).condition() > (*i).condition()))
				problem_solver.evaluate_condition(II, EE, (*i).condition());
			if ((*II).value() != (*i).value()) result.add_condition_back(*i);
			++II;
			++i;
		}
		else
		{
			if ((*I).value() == (*i).value()) result.add_condition_back(*i);
			++I;
			++i;
		}
	}
...
{
	if (m_weight_actual) return (m_min_weight);
	...
	for (; (i != e) && (I != E);)
	{
		...
		{
			if ((*I).value() != (*i).value()) ++m_min_weight;
			++I;
			++i;
		}
	}
	if (i != e) m_min_weight = m_min_weight + _edge_value_type(e - i);
