...
void game_sv_Single::Create(shared_str& options)
{
	...
	if (strstr(*options, "/alife")) m_alife_simulator = xr_new<CALifeSimulator>(&server(), &options);
...
void game_sv_Single::OnCreate(u16 id_who)
{
	...
	VERIFY(e_who);
	if (!e_who->m_bALifeControl) return;
...
void game_sv_Single::OnDetach(u16 eid_who, u16 eid_what)
{
	if (ai().get_alife())
	{
		...
		CSE_ALifeInventoryItem* l_tpALifeInventoryItem = smart_cast<CSE_ALifeInventoryItem*>(e_what);
		if (!l_tpALifeInventoryItem) return;

		CSE_ALifeDynamicObject* l_tpDynamicObject = smart_cast<CSE_ALifeDynamicObject*>(e_who);
		if (!l_tpDynamicObject) return;
...
ALife::_TIME_ID game_sv_Single::GetEnvironmentGameTime() { return (inherited::GetGameTime()); }
...
{
	if (!ai().get_alife()) return (inherited::load_game(net_packet, sender));
...
{
	if (!ai().get_alife()) return (inherited::level_name(server_options));
