...
TEMPLATE_SPECIALIZATION
IC const _index_type& CSolverPathManager::get_value(_Graph::const_iterator& i, bool reverse_search) const
{
	return (graph->value(*best_node_index, i, reverse_search));
}

TEMPLATE_SPECIALIZATION
IC const typename CSolverPathManager::_edge_type& CSolverPathManager::edge(_Graph::const_iterator& i) const
{
...
{
	VERIFY(this->data_storage);
	if (m_edge_path) data_storage->get_edge_path(*m_edge_path, &vertex, CSolverPathManager::_Graph::reverse_search);
}
