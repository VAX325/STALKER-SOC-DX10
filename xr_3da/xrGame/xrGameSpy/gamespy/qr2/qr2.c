...
#define AVAILABLE_BUFFER_LEN(a) (MAX_DATA_SIZE - (a)->len)
	...
	********/
								 ...
								 qr2_adderrorcallback_t adderror_callback, void* userdata)
	{
		...
		if (qrec == NULL) { cr = &static_qr2_rec; }
		else
		{
		...
		if (num_local_ips == 0) enum_local_ips();
		if (ispublic)
		{
			int override = qr2_hostname[0];
			if (!override) sprintf(hostname, "%s.master." GSI_DOMAIN_NAME, gamename);
	...
	int saddrlen;
		...
		while (baseport < maxport)
		{
			...
			if (lasterror == 0) break; // we found a port
			baseport++;
		}
	...
	{
		if (qrec == NULL) qrec = current_rec;
		if (qrec->ispublic) qr2_check_send_heartbeat(qrec);
	...
	int saddrlen = sizeof(struct sockaddr_in);
		...
		if (!qrec->read_socket) return; // not our job
	...
	void qr2_shutdown(qr2_t qrec)
	{
		...
		if (qrec == NULL) qrec = current_rec;
		if (qrec->ispublic) send_heartbeat(qrec, 2);
	...
	gsi_bool qr2_keybuffer_add(qr2_keybuffer_t keybuffer, int keyid)
	{
		...
		if (keybuffer->numkeys >= MAX_REGISTERED_KEYS) return gsi_false;
		if (keyid < 1 || keyid > MAX_REGISTERED_KEYS) return gsi_false;
	...
	gsi_bool qr2_buffer_addA(qr2_buffer_t outbuf, const char* value)
	{
		...
		GS_ASSERT(value)
		{
			...
			if (copylen <= 0) return gsi_false;         // no space
	...
	static void enum_local_ips()
	{
		...
		{
			if (phost->h_addr_list[num_local_ips] == 0) break;
	...
	static int get_sockaddrin(const char* host, int port, struct sockaddr_in* saddr, struct hostent** savehent)
	{
		...
		if (savehent != NULL) *savehent = hent;
		return 1;
	}
...
#define MAX_CHALLENGE 64
	static void compute_challenge_response(qr2_t qrec, qr2_buffer_t buf, char* challenge, int challengelen)
	{
		...
		if (challengelen < 1) return;                   // invalid, need room for the NUL
		if (challengelen > (MAX_CHALLENGE + 1)) return; // invalid
		if (challenge[challengelen - 1] != 0) return;   // invalid - must be NTS
											 ...
											 uchar* keys)
	{
		...
		kb.numkeys = 0;
		if (keycount == 0) return; // no keys wanted

		if (keytype == key_player || keytype == key_team) // need to add the player/team counts
		{
			if (AVAILABLE_BUFFER_LEN(buf) < sizeof(cttemp)) return; // no more space
		...
		if (keycount == 0xFF) // need to get the list of keys
		{
			...
			for (i = 0; i < kb.numkeys; i++)
			{
				...
				if (keytype == key_server) // add the server values
				{
					...
					if (len == buf->len) qr2_buffer_addA(buf, "");
				}
			}
			// add an extra null
			if (AVAILABLE_BUFFER_LEN(buf) < 1) return; // no space
			buf->buffer[buf->len++] = 0;
			keycount = kb.numkeys;
			keys = kb.keys;
			if (keytype == key_server) return; // already added the keys
		}
	...
	static gsi_bool qr_build_split_query_reply(qr2_t qrec, qr2_buffer_t buf, struct QRSplitQueryProgress* progress)
	{
		...
		if (progress->mCurKeyType < 0 || progress->mCurKeyType >= key_type_count) return gsi_false; // stop processing

		// check buffer space
		//    (buffer should only contain header at this point)
		if (AVAILABLE_BUFFER_LEN(buf) < 32) return gsi_false; // no space?
		...
		*packetNumPos = (gsi_u8)progress->mCurPacketNum++;
		...
		while (progress->mCurKeyType < key_type_count)
		{
			...
			// check buffer space
			if (AVAILABLE_BUFFER_LEN(buf) < 100) return gsi_true; // no space
			...
			while (progress->mCurKeyIndex < progress->mKeyBuffer.numkeys)
			{
				...
				// Write the key name
				if (gsi_is_false(qr2_buffer_addA(buf, aKeyName))) return gsi_true; // send, then try again
				...
				else
				{
					...
					while (progress->mCurSubIndex < progress->mCurSubCount)
					{
						...
						// make sure the key was written
						if (AVAILABLE_BUFFER_LEN(buf) < 1) return gsi_true; // ran out of space, try again next packet

						// move onto the next player/team value
						progress->mCurSubIndex++;
					}
					// append a null to signify end of this team/player key
					if (AVAILABLE_BUFFER_LEN(buf) > 0) buf->buffer[buf->len++] = '\0';
				}
				// move onto next key
	...
	*/
	...
	static void qr_process_client_message(qr2_t qrec, char* buf, int len)
	{
		...
		if (isnatneg)
		{
			...
			if (qrec->nn_callback) qrec->nn_callback((int)ntohl((unsigned int)cookie), qrec->udata);
		}
	...
	static int qr_got_recent_message(qr2_t qrec, int msgkey)
	{
		...
		for (i = 0; i < RECENT_CLIENT_MESSAGES_TO_TRACK; i++)
		{
			if (qrec->client_message_keys[i] == msgkey) return 1;
		}
	...
	void qr2_parse_queryA(qr2_t qrec, char* query, int len, struct sockaddr* sender)
	{
		...
		if (len < 7) return; // too small to be valid
		// check the magic...
		if ((uchar)query[0] != QR_MAGIC_1 || (uchar)query[1] != QR_MAGIC_2) return;
...
// #define SIMULATE_FIREWALL
		...
		if (qrec->listed_state > 0) qrec->listed_state = 0;
		...
		switch (ptype)
		{
			...
			case PACKET_QUERY:
				...
				{
					if (len < 4) return; // too small for an ip-verify query
			...
			case PACKET_CHALLENGE:
				...
				for (i = 0; i < REQUEST_KEY_LEN; i++)
				{
					if (reqkey[i] != qrec->instance_key[i]) return; // not a valid instance key
				}
			...
			case PACKET_ADDERROR:
				...
				if (qrec->listed_state == -1) return; // we already got an error message
				// verify the instance code
				for (i = 0; i < REQUEST_KEY_LEN; i++)
				{
					if (reqkey[i] != qrec->instance_key[i]) return; // not a valid instance key
				}
				if (len < 2) return; // not a valid message
				qrec->listed_state = -1;
#ifndef GSI_UNICODE
			...
			case PACKET_CLIENT_MESSAGE:
				...
				if (!qr_got_recent_message(qrec, i)) qr_process_client_message(qrec, pos + 4, len - 4);
	...
	static void send_heartbeat(qr2_t qrec, int statechanged)
	{
		...
		else
		{
			...
			if (AVAILABLE_BUFFER_LEN(&buf) >= 1) buf.buffer[buf.len++] = 0;
		}
		...
		if (statechanged != 0) qrec->userstatechangerequested = 0;
