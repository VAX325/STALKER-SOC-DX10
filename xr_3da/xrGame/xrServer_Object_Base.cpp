...
CSE_Abstract::CSE_Abstract(LPCSTR caSection)
{
	...
	if (pSettings->line_exist(caSection, "custom_data"))
	{
		...
		if (!FS.exist(file_name)) { Msg("! cannot open config file %s", file_name); }
		else
		{
...
#pragma warning(disable : 4238)
		auto reader = IReader((void*)(*(m_ini_string)), m_ini_string.size());
...
BOOL CSE_Abstract::Spawn_Read(NET_Packet& tNetPacket)
{
	...
	// dangerous!!!!!!!!!
	if (s_flags.is(M_SPAWN_VERSION)) tNetPacket.r_u16(m_wVersion);
	...
	if (m_wVersion > 69) m_script_version = tNetPacket.r_u16();
	...
	if (m_wVersion > 79) tNetPacket.r(&m_tSpawnID, sizeof(m_tSpawnID));
...
void CSE_Abstract::FillProps(LPCSTR pref, PropItemVec& items)
{
	...
	// flSpawnIfDestroyedOnly);
	//	PHelper().CreateFlag32		(items,PrepareKey(pref,*s_name,"Spawn\\spawn infinite count"),		&m_spawn_flags,
	// flSpawnInfiniteCount); 	PHelper().CreateFlag32		(items,PrepareKey(pref,*s_name,"Spawn\\auto destroy on
	// spawn"), &m_spawn_flags,			flSpawnDestroyOnSpawn);
#endif // DEBUG
