...
IC void CBucketList::init()
{
	...
	if (clear_buckets) ZeroMemory(m_buckets, bucket_count * sizeof(CGraphVertex*));
}
...
{
	if (m_min_bucket_id == bucket_count) return (true);
...
{
	if (vertex.f() >= m_max_bucket_value) return (bucket_count - 1);
	if (vertex.f() <= m_min_bucket_value) return (0);
...
{
	if (m_bucket_id < m_min_bucket_id) m_min_bucket_id = m_bucket_id;
...
IC void CBucketList::remove_best_opened()
{
	...
	if (m_buckets[m_min_bucket_id]) m_buckets[m_min_bucket_id]->prev() = 0;
	verify_buckets();
}
