								  ...
								  const Fvector& expl_centre, const float expl_radius)
{
	...
	if (l_b1.contains(local_exp_center)) return 1.f;
	Fvector l_c, l_d;
	l_b1.get_CD(l_c, l_d);
...
#ifdef DEBUG
	if (ph_dbg_draw_mask.test(phDbgDrawExplosions)) { Msg("damage effect %f", effect / TEST_RAYS_PER_OBJECT); }
#endif
...
void CExplosive::Explode()
{
	...
	for (u32 o_it = 0; o_it < ISpatialResult.size(); o_it++)
	{
		...
		if (pGameObject && cast_game_object()->ID() != pGameObject->ID()) m_blasted_objects.push_back(pGameObject);
	}
	...
	//---------------------------------------------------------------------
#ifdef DEBUG
	if (ph_dbg_draw_mask.test(phDbgDrawExplosions)) { DBG_ClosedCashedDraw(100000); }
#endif
	//////////////////////////////////////////////////////////////////////////
	// Explode Effector	//////////////
...
void CExplosive::UpdateCL()
{
	...
	else
	{
		...
		{
			if (m_fExplodeHideDurationMax <= (m_fExplodeDurationMax - m_fExplodeDuration)) { HideExplosive(); }
		}
		UpdateExplosionPos();
...
{
	if (exp_obj) { exp_obj->GetRayExplosionSourcePos(p); }
}
...
#ifdef DEBUG
	if (ph_dbg_draw_mask.test(phDbgDrawExplosions)) { DBG_OpenCashedDraw(); }
#endif
...
void CExplosive::net_Relcase(CObject* O)
{
	...
	{
		if (O->ID() == m_iCurrentParentID) m_iCurrentParentID = u16(-1);
	}

	BLASTED_OBJECTS_I I =
		std::find(m_blasted_objects.begin(), m_blasted_objects.end(), smart_cast<CPhysicsShellHolder*>(O));
	if (m_blasted_objects.end() != I) { m_blasted_objects.erase(I); }
}
