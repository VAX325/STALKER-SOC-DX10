...
{
#ifdef USE_DEBUGGER
	if (debugger()) debugger()->PrepareLuaBind();
#endif
...
void CScriptEngine::process_file_if_exists(LPCSTR file_name, bool warn_if_not_exist)
{
	...
	if (!warn_if_not_exist && no_file_exists(file_name, string_length)) return;
...
{
	if (!xr_strlen(function_to_call)) return (false);

	string256 name_space, function;

	parse_script_namespace(function_to_call, name_space, function);
	if (xr_strcmp(name_space, "_G")) process_file(name_space);

	if (!this->object(name_space, function, type)) return (false);
...
{
	if (m_last_no_file_length != string_length) return (false);
...
{
	if (NULL == this || NULL == lua()) return "#ERROR: Script engine not ready";
	...
	if (lua_isfunction(lua(), -1))
	{
		...
		if (0 != lua_pcall(lua(), args, LUA_MULTRET, 0)) lua_pcall_failed(lua());

		static string1024 result;
		strcpy_s(result, "#OK");
		if (lua_isstring(lua(), -1)) strcpy_s(result, 1023, lua_tostring(lua(), -1));
...
{
	if (!g_game_lua) g_game_lua = ai().script_engine().lua();
	return g_game_lua;
}
