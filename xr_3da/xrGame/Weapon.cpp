...
void CWeapon::Load(LPCSTR section)
{
	...
	else { camRelaxSpeed_AI = camRelaxSpeed; }
...
int CWeapon::GetAmmoCurrent(bool use_item_to_spawn) const
{
	...
	if (m_pCurrentInventory->ModifyFrame() <= m_dwAmmoCurrentCalcFrame) return l_count + iAmmoCurrent;
	...
	for (int i = 0; i < (int)m_ammoTypes.size(); ++i)
	{
		...
		if (!use_item_to_spawn) continue;

		if (!inventory_owner().item_to_spawn()) continue;
...
bool CWeapon::ScopeAttachable() { return (CSE_ALifeItemWeapon::eAddonAttachable == m_eScopeStatus); }
bool CWeapon::SilencerAttachable() { return (CSE_ALifeItemWeapon::eAddonAttachable == m_eSilencerStatus); }
...
{ // actor only
	...
	if (ScopeAttachable())
	{
		...
		if (IsScopeAttached())
		{
			if (FALSE == pHudVisual->LL_GetBoneVisible(bone_id)) pHudVisual->LL_SetBoneVisible(bone_id, TRUE, TRUE);
		}
		else
		{
			if (pHudVisual->LL_GetBoneVisible(bone_id)) pHudVisual->LL_SetBoneVisible(bone_id, FALSE, TRUE);
		}
	}
	...
	{
		if (bone_id == BI_NONE) bone_id = pHudVisual->LL_BoneID(wpn_launcher);
...
float CWeapon::CurrentZoomFactor() { return IsScopeAttached() ? m_fScopeZoomFactor : m_fIronSightZoomFactor; };
...
{
	if (GetAmmoCurrent(true) || m_ammoTypes.empty()) return (true);
...
float CWeapon::Weight()
{
	...
	if (IsScopeAttached() && GetScopeName().size()) { res += pSettings->r_float(GetScopeName(), "inv_weight"); }
...
void CWeapon::Show() { SwitchState(eShowing); }

bool CWeapon::show_crosshair() { return !IsZoomed(); }
