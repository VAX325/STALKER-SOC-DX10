...
{
	if ((o1.type() == LUA_TNIL) && (o2.type() == LUA_TNIL)) return (true);
...
public:
	...
	{
		clear();
		if (callback.m_functor.is_valid() && callback.m_functor.lua_state()) m_functor = callback.m_functor;
		if (callback.m_object.is_valid() && callback.m_object.lua_state()) m_object = callback.m_object;
		return *this;
	}
			...
			try
			{
				if (m_functor)
				{
					...
					if (m_object)
					{
						...
						// return TResult(luabind::call_function<TResult>(m_functor, m_object,
						// std::forward<Args>(args)...));
						return m_functor(m_object, std::forward<Args>(args)...);
					}
					return m_functor(
						std::forward<Args>(args)...); // return TResult(luabind::call_function<TResult>(m_functor,
													  // std::forward<Args>(args)...));
				}
			}
		...
		try
		{
			if (m_functor)
			{
				...
				if (m_object)
				{
					...
					m_functor(std::forward<Args>(
						args)...); // luabind::call_function<void>(m_functor, std::forward<Args>(args)...);
			}
		}
