							   ...
							   float maximum_distance, const CCartridge& cartridge, bool SendHit, bool AimBullet)
{
	...
	if (SendHit && GameID() != GAME_SINGLE) Game().m_WeaponUsageStatistic->OnBullet_Fire(&bullet, cartridge);
	m_Lock.Leave();
}
								...
								u32 delta_time)
{
	...
	if (result && bullet_data.bStopTracing) { range = (rq_storage.r_begin() + rq_storage.r_count() - 1)->range; }
	...
	if (!bullet->flags.ricochet_was)
	{
		...
		if (bullet->fly_dist >= bullet->max_dist) return false;
...
#pragma todo("а как насчет bullet->speed==0")
	...
	if (bullet->speed < m_fMinBulletSpeed) return false;
...
{
#if defined(DEBUG) and defined(FUCKING_SHIT)
	if (g_bDrawBulletHit && !m_bullet_points.empty())
	{
		...
		if (m_bullet_points.size() > 32768) m_bullet_points.clear_not_free();
	}
	else
	...
	for (BulletVecIt it = m_BulletsRendered.begin(); it != m_BulletsRendered.end(); it++)
	{
		...
		if (length > m_fTracerLengthMax) length = m_fTracerLengthMax;
		...
		bool bActor = false;
		if (Level().CurrentViewEntity()) { bActor = (bullet->parent_id == Level().CurrentViewEntity()->ID()); }
		...
		case EVENT_HIT:
			{
				...
				if (_dynamic)
				{
					...
					E.Repeated = (R.O->ID() == E.bullet.targetID);
					if (GameID() == GAME_SINGLE) { bullet->targetID = R.O->ID(); }
					else
					{
						if (bullet->targetID != R.O->ID())
						{
							CGameObject* pGO = smart_cast<CGameObject*>(R.O);
							if (!pGO || !pGO->BonePassBullet(R.element)) bullet->targetID = R.O->ID();
						}
					}
				};
			}
