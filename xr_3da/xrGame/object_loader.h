...
struct CLoader
{
	...
	template <typename T>
	struct CHelper2<T, false>
	{
		IC static void load_data(T& data, M& stream, const P& p)
		{
			constexpr bool is_base_or_derived =
				object_type_traits::is_base_and_derived_or_same_from_template<IPureLoadableObject, T>::value;
			CHelper1<T, is_base_or_derived>::load_data(data, stream, p);
		}
	};

	template <typename T>
	struct CHelper2<T, true>
	{
		IC static void load_data(T& data, M& stream, const P& p)
		{
			CLoader<M, P>::load_data(*(data = xr_new<typename object_type_traits::remove_pointer<T>::type>()), stream,
									 p);
		}
	};
	...
	struct CHelper3
	{
		...
		struct add_helper<T1, T2, false>
		{
			IC static void add(T1& data, T2& value) { data.push_back(value); }
		};

		template <typename T1, typename T2>
		struct add_helper<T1, T2, true>
		{
			IC static void add(T1& data, T2& value) { data.insert(value); }
		};

		template <typename T1, typename T2>
		IC static void add(T1& data, T2& value)
		{
			add_helper<T1, T2, is_tree_structure<T1>::value>::add(data, value);
		}
	...
	template <typename T>
	struct CHelper4<T, false>
	{
		IC static void load_data(T& data, M& stream, const P& p)
		{
			constexpr bool is_ptr = object_type_traits::is_pointer<T>::value;
			CHelper2<T, is_ptr>::load_data(data, stream, p);
		}
	};
	...
	{
		IC static void load_data(T& data, M& stream, const P& p) { CHelper3::load_data(data, stream, p); }
	};
	...
	IC static void load_data(T& data, M& stream, const P& p)
	{
		constexpr bool is_stl = object_type_traits::is_stl_container<T>::value;
		CHelper4<T, is_stl>::load_data(data, stream, p);
	}
};
