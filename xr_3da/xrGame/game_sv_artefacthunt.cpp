...
void game_sv_ArtefactHunt::OnRoundStart()
{
	...
	Artefact_PrepareForSpawn();
	//-------------------------------------------------------
	if (Get_ReinforcementTime() == -1) { RespawnAllNotAlivePlayers(); };
}
...
KILL_RES game_sv_ArtefactHunt::GetKillResult(game_PlayerState* pKiller, game_PlayerState* pVictim)
{
	...
	switch (Res)
	{
		case KR_TEAMMATE:
			{
				if (pVictim->GameID == artefactBearerID) Res = KR_TEAMMATE_CRITICAL;
			}
			break;
		case KR_RIVAL:
			{
				if (pVictim->GameID == artefactBearerID) Res = KR_RIVAL_CRITICAL;
			}
			break;
		default:
			{
											  ...
											  CSE_Abstract* pWeaponA)
{
	...
	if (ps_killed && ps_killed->GameID == m_iAfBearerMenaceID) m_iAfBearerMenaceID = 0;
};
...
void game_sv_ArtefactHunt::OnPlayerBuySpawn(ClientID sender)
{
	...
	if (!xrCData->ps->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD)) { xrCData->ps->m_bPayForSpawn = false; }
};
										 ...
										 xr_vector<u32>& rpIDEnemy, xr_vector<u32>& EnemyIt, bool force_find)
{
	...
	for (u32 p = 0; p < rps.size(); ++p)
	{
		...
		if (Blocked || rp.Blocked) { continue; };
		dest.push_back(p);
	}
...
{
	if (Get_ReinforcementTime() == -1 || Get_ReturnPlayers()) { MoveAllAlivePlayers(); };
	if (Get_ReinforcementTime() > 0 || Get_ReinforcementTime() == -1) { RespawnAllNotAlivePlayers(); };
	//-----------------------------------------------------------
		...
		case GAME_PHASE_TEAM2_ELIMINATED:
			{
				if (m_delayedTeamEliminated && m_TeamEliminatedDelay < Device.TimerAsync())
				{
					...
					if (Get_ReturnPlayers()) { MoveAllAlivePlayers(); }
...
void game_sv_ArtefactHunt::OnCreate(u16 id_who)
{
	...
	if (pIArtefact) m_dwArtefactID = pIArtefact->ID;
	CSE_ALifeTeamBaseZone* pTeamBase = smart_cast<CSE_ALifeTeamBaseZone*>(pEntity);
	if (pTeamBase && m_bSwapBases) { pTeamBase->m_team = 3 - pTeamBase->m_team; }
};
...
void game_sv_ArtefactHunt::CheckForTeamWin()
{
	...
	if (!WinTeam)
	{
		...
		if (GetTimeLimit() && ((Level().timeServer() - StartTime())) > u32(GetTimeLimit() * 60000))
		{
			...
			if (Team1 == Team2)
			{
				if (!g_sv_Skip_Winner_Waiting) { return; };
				WinTeam = 1;
			}
			else { WinTeam = (Team1 > Team2) ? 1 : 2; }
		}
		else { return; }
	};
...
void game_sv_ArtefactHunt::OnPlayerHitPlayer(u16 id_hitter, u16 id_hitted, NET_Packet& P)
{
	...
	if (ps_hitted->GameID == artefactBearerID) m_iAfBearerMenaceID = ps_hitter->GameID;
};
