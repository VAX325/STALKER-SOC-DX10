...
void CSE_ALifeDynamicObject::on_register()
{
	...
	if (!alife().graph().level().object(object->ID, true) && !keep_saved_data_anyway()) client_data.clear();
}
...
void CSE_ALifeDynamicObject::try_switch_online()
{
	...
	if (schedulable)
	{
		...
		{
			if (alife().scheduled().object(ID, true)) alife().scheduled().remove(this);
		}
...
void CSE_ALifeDynamicObject::try_switch_offline()
{
	...
	if (alife().graph().actor()->o_Position.distance_to(o_Position) <= alife().offline_distance()) return;
...
void CSE_InventoryBox::add_online(const bool& update_registries)
{
	...
	for (; I != E; ++I)
	{
		...
		l_tpALifeInventoryItem->base()->s_flags.m_or(M_SPAWN_UPDATE);
		...
		l_tpALifeDynamicObject->s_flags.m_and(u16(-1) ^ M_SPAWN_UPDATE);
...
#endif // DEBUG
		if (!child->keep_saved_data_anyway()) child->client_data.clear();
