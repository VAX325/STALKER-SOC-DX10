...
game_PlayerState* game_sv_GameState::get_eid(u16 id) // if exist
{
	...
	if (entity)
	{
		if (entity->owner)
		{
			if (entity->owner->ps)
			{
				if (entity->owner->ps->GameID == id) return entity->owner->ps;
			}
		}
	}
	//-------------------------------------------------
...
void game_sv_GameState::net_Export_State(NET_Packet& P, ClientID to)
{
	...
	for (u32 p_it = 0; p_it < get_players_count(); ++p_it)
	{
		...
		else
		{
			...
			if (0 == C_e)
				strcpy(p_name, "Unknown");
			else { strcpy(p_name, C_e->name_replace()); }
		}

		A->setName(p_name);
		u16 tmp_flags = A->flags__;

		if (Base == A) A->setFlag(GAME_PLAYER_FLAG_LOCAL);
...
void game_sv_GameState::net_Export_Update(NET_Packet& P, ClientID id_to, ClientID id)
{
	...
	if (A)
	{
		...
		if (id == id_to) { A->setFlag(GAME_PLAYER_FLAG_LOCAL); }
					...
					case rptActorSpawn:
						{
							...
							for (int i = 0; i < int(rpoints[team].size()) - 1; i++)
							{
								...
								if (dist < rpoints_MinDist[team]) rpoints_MinDist[team] = dist;
								dist = R.P.distance_to(rp.P) / 2;
								if (dist < rpoints_Dist[team]) rpoints_Dist[team] = dist;
							};
						}
						break;
				};
...
void game_sv_GameState::ReadOptions(shared_str& options)
{
	...
	if (FS.exist(MAPROT_LIST)) Console->ExecuteScript(MAPROT_LIST);
	...
	{
		if (g_sv_base_iVotingEnabled == 1) g_sv_base_iVotingEnabled = 0x00ff;
	}
};
//-----------------------------------------------------------
...
void game_sv_GameState::assign_RP(CSE_Abstract* E, game_PlayerState* ps_who)
{
	...
	u32 rpoint = 0;
	if (xrp.size() && !tpSpectator) { rpoint = xrp[::Random.randI((int)xrp.size())]; }
	else
	{
		if (!tpSpectator)
		{
	...
	//-----------------------------------------------------------
	RPoint& r = rp[rpoint];
	if (!tpSpectator) { r.TimeToUnfreeze = Level().timeServer() + g_sv_base_dwRPointFreezeTime; };
...
void game_sv_GameState::Update()
{
	...
	if (!g_dedicated_server)
	{
		if (Level().game)
		{
			...
			if (script_process) script_process->update();
		}
	}
}
...
game_sv_GameState::~game_sv_GameState()
{
	if (!g_dedicated_server) ai().script_engine().remove_script_process(ScriptEngine::eScriptProcessorGame);
...
void game_sv_GameState::OnRoundEnd()
{
	...
	else { m_bMapNeedRotation = true; }

	m_bFastRestart = false;
	if (round_end_reason == eRoundEnd_GameRestartedFast) { m_bFastRestart = true; }
} // конец раунда
