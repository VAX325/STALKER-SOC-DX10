...
void CCustomRocket::activate_physic_shell()
{
	...
	create_physic_shell();
	if (m_pPhysicsShell->isActive()) return;
...
void CCustomRocket::StopEngine()
{
	...
	if (m_bStopLightsWithEngine) StopLights();
...
void CCustomRocket::UpdateEngine()
{
	//	VERIFY( getVisible() );
	//	VERIFY( m_pPhysicsShell);
	if (!m_pPhysicsShell) Msg("! CCustomRocket::UpdateEngine called, but 0==m_pPhysicsShell");
...
{
	if (m_flyingSound._handle() && m_flyingSound._feedback()) m_flyingSound.set_position(XFORM().c);
...
{
	if (m_flyingSound._handle()) m_flyingSound.play_at_pos(0, XFORM().c, sm_Looped);
...
{
	if (m_flyingSound._handle()) m_flyingSound.stop();
...
void CCustomRocket::OnEvent(NET_Packet& P, u16 type)
{
	switch (type)
	{
		case GE_GRENADE_EXPLODE:
			{
				if (m_eState != eCollide && OnClient()) { CCustomRocket::Contact(Position(), Direction()); };
			}
			break;
	}
