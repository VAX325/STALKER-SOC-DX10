	...
	else
	{
		if (m_pCurrentDialog->IsWeSpeaking(m_pOurDialogManager))
		{
			...
			if (m_pCurrentDialog && !m_pCurrentDialog->allIsDummy())
			{
				...
				{
					CPhrase* phrase = *it;
					AddQuestion(m_pCurrentDialog->GetPhraseText(phrase->GetID()), phrase->GetID(), number, false);
				}
			}
			else
				UpdateQuestions();
...
{
	if (pWnd == UITalkDialogWnd && msg == TALK_DIALOG_TRADE_BUTTON_CLICKED) { SwitchToTrade(); }
	else if (pWnd == UITalkDialogWnd && msg == TALK_DIALOG_QUESTION_CLICKED) { AskQuestion(); }
...
void CUITalkWnd::Update()
{
	...
	if (g_actor && m_pActor && !m_pActor->IsTalking()) { Game().StartStopMenu(this, true); }
	else
	{
	...
	if (m_bNeedToUpdateQuestions) { UpdateQuestions(); }
	inherited::Update();
...
void CUITalkWnd::AskQuestion()
{
	...
	else { phrase_id = UITalkDialogWnd->m_ClickedQuestionID; }
...
bool CUITalkWnd::IR_OnKeyboardPress(int dik)
{
	...
	if (cmd == kUSE)
	{
		if (m_pOthersInvOwner && m_pOthersInvOwner->NeedOsoznanieMode()) { return true; }
