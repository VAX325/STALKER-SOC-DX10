...
CUIMapList::~CUIMapList() { delete_data(m_pExtraContentFilter); }
...
void CUIMapList::Init(float x, float y, float width, float height) { CUIWindow::Init(x, y, width, height); }
...
void CUIMapList::SendMessage(CUIWindow* pWnd, s16 msg, void* pData)
{
	...
	{
		if (pWnd == m_pList1) OnListItemClicked();
	}
}
...
const char* CUIMapList::GetCLGameModeName() { return get_token_name(game_types, GetCurGameType()); }
...
void CUIMapList::LoadMapList()
{
	...
	if (game_weathers.size()) m_pWeatherSelector->SetItem(0);
}
...
void CUIMapList::SetModeSelector(CUISpinText* ms) { m_pModeSelector = ms; }
...
void CUIMapList::OnBtnLeftClick() { m_pList2->RemoveWindow(m_pList2->GetSelected()); }
