...
void CUIMainIngameWnd::SetAmmoIcon(const shared_str& sect_name)
{
	...
	if (iGridWidth < 2) x += (UIWeaponIcon_rect.width() - w) / 2.0f;
		...
		while (i < ewiInvincible)
		{
			...
			switch (i)
			{
				...
				case ewiWeaponJammed:
					if (m_pWeapon) value = 1 - m_pWeapon->GetConditionToShow();
					break;
				case ewiStarvation:
...
bool CUIMainIngameWnd::OnKeyboardPress(int dik)
{
	...
	if (g_bHudAdjustMode)
	{
		...
		if (1 == g_bHudAdjustMode) // zoom offset
		{
			...
			if (tmpV.x || tmpV.y || tmpV.z) pWpnHud->SetZoomOffset(tmpV);
		}
...
void CUIMainIngameWnd::AnimateContacts(bool b_snd)
{
	...
	if (b_snd) HUD_SOUND::PlaySound(m_contactSnd, Fvector().set(0, 0, 0), 0, true);
}
