...
struct SPHContactDBGDraw : public SPHDBGDrawAbsract
{
	...
	SPHContactDBGDraw(const dContact& c)
	{
		...
		if (dGeomGetBody(c.geom.g1)) { geomClass = dGeomGetClass(retrieveGeom(c.geom.g1)); }
		else { geomClass = dGeomGetClass(retrieveGeom(c.geom.g2)); }
...
struct SPHDBGOutText : public SPHDBGDrawAbsract
{
	...
	virtual void render()
	{
		...
		{
			HUD().Font().pFontStat->SetHeight(HUD().Font().pFontStat->GetHeight() * dbg_text_height_scale /
...
struct SPHDBGTextSetColor : public SPHDBGDrawAbsract
{
	...
	virtual void render() { HUD().Font().pFontStat->SetColor(color); }
};
...
struct SPHDBGTextOutSet : public SPHDBGDrawAbsract
{
	...
	virtual void render() { HUD().Font().pFontStat->OutSet(x, y); }
};
...
void DBG_DrawPHAbstruct(SPHDBGDrawAbsract* a)
{
	...
	switch (dbg_ph_draw_mode)
	{
		case dmSecondaryThread:
			if (draw_frame) { push(dbg_draw_abstruct0, a); }
			else { push(dbg_draw_abstruct1, a); };
			break;
		case dmCashed:
...
void DBG_PHAbstruactStartFrame(bool dr_frame)
{
	...
	if (dr_frame) { dbg_draw_abstruct0.clear(); }
	else { dbg_draw_abstruct1.clear(); }
}
...
void DBG_PHAbstructRender()
{
	...
	if (dbg_ph_draw_mode != dmCashed)
	{
		...
		if (cash_draw_remove_time < Device.dwTimeGlobal) { clear_vector(dbg_draw_cashed); }
	}
	{
		PHABS_DBG_I i, e;
...
void DBG_AnimBlend(IKinematicsAnimated& ka, const CBlend& B)
{
	...
	if (dbg_track_obj_flags.test(dbg_track_obj_blends_state))
		DBG_OutText("state: %s, playing: %s, stop_at_end_callback: %s ", name_blend_type(B.blend), name_bool(B.playing),
...
static void DBG_DrawTarckObj()
{
	if (!ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject)) return;
...
bool is_trace_obj(CPHObject* obj) { return trace_object && static_cast<CObject*>(obj->ref_object()) == trace_object; }
