...
static oaBool ParseInitStr(const oaChar* init_str, oaChar* plugin_path, oaChar* opt);
	...
	oaBool oaInit(const oaChar* init_str, oaVersion* version)
	{
			 ...
			 (PluginVersion.Minor < OA_MIN_REQUIRED_VERSION_MINOR)))
		{
			...
			sprintf(ErrorStr,
					...
					"Plugin version is %ld.%ld.",
					OA_MIN_REQUIRED_VERSION_MAJOR, OA_MIN_REQUIRED_VERSION_MINOR, PluginVersion.Major,
					PluginVersion.Minor);
		...
		memcpy(&FuncTable, PluginFuncTable, OA_MIN(sizeof(FuncTable), PluginFuncTable->TableSize));
...
#define INIT_CHECK()                                                                                                   \
	...
	oaCommandType oaGetNextCommand(oaCommand* command)
	{
		...
		if (NextCommand == OA_CMD_EXIT) Cleanup();
	...
	oaNamedOption* oaGetNextOption(void)
	{
		...
		if (FuncTable.GetNextOption) return (FuncTable.GetNextOption());
	...
	void oaAddOption(const oaNamedOption* option)
	{
		...
		if (FuncTable.AddOption) FuncTable.AddOption(option);
	}
	...
	void oaAddOptionValue(const oaChar* name, oaOptionDataType value_type, const oaValue* value)
	{
		...
		if (FuncTable.AddOptionValue) FuncTable.AddOptionValue(name, value_type, value);
	}
	...
	void oaAddBenchmark(const oaChar* benchmark_name)
	{
		...
		if (FuncTable.AddBenchmark) FuncTable.AddBenchmark(benchmark_name);
	}
	...
	void oaAddResultValue(const oaChar* name, oaOptionDataType value_type, const oaValue* value)
	{
		...
		if (FuncTable.AddResultValue) FuncTable.AddResultValue(name, value_type, value);
	}
	...
	oaBool oaSendSignal(oaSignalType signal, void* param)
	{
		...
		if (FuncTable.SendSignal) return (FuncTable.SendSignal(signal, param));
	...
	void oaStartBenchmark(void)
	{
		...
		if (FuncTable.StartBenchmark) FuncTable.StartBenchmark();
	}
	...
	void oaDisplayFrame(oaFloat t)
	{
		...
		if (FuncTable.DisplayFrame) FuncTable.DisplayFrame(t);
	}
	...
	void oaEndBenchmark(void)
	{
		...
		if (FuncTable.EndBenchmark) FuncTable.EndBenchmark();
	}
...
static oaBool ParseInitStr(const oaChar* init_str, oaChar* plugin_path, oaChar* opt)
{
	...
	if (InitStr[i] == ';') { OA_STRCPY(opt, InitStr + i + 1); }
	else
		opt[0] = 0;
...
static oaBool LoadPlugin(void)
{
	...
	int Length =
		MultiByteToWideChar(CP_UTF8, 0, PluginPath, (int)OA_STRLEN(PluginPath) + 1, WidePluginPath, OA_MAX_PATH_LENGTH);
	...
	if (Length == 0)
	{
		...
		if (OA_STRLEN(PluginPath) == 0) OA_ERROR("OpenAutomate PluginPath was undefined\n");
...
void oaAddFrameValue(const oaChar* name, oaOptionDataType value_type, const oaValue* value)
{
	...
	if (FuncTable.AddFrameValue) FuncTable.AddFrameValue(name, value_type, value);
}
