...
#define STAB_PRIM(prim)                                                                                                \
	...
	{                                                                                                                  \
		...
		{                                                                                                              \
			mNbIntersections++;                                                                                        \
			if (IR(mStabbedFace.mDistance) < IR(mMaxDist)) { HANDLE_CONTACT(prim) }                                    \
		}                                                                                                              \
	}
#else
#define STAB_PRIM(prim)                                                                                                \
...
#endif
	  mNbRayBVTests(0), mNbRayPrimTests(0), mNbIntersections(0), mMaxDist(flt_max), mClosestHit(false), mCulling(true)
{
}
...
void RayCollider::_Stab(const AABBCollisionNode* node)
{
	...
	if (node->IsLeaf()) { STAB_PRIM(node->GetPrimitive()) }
	else
	{
		_Stab(node->GetPos());
...
void RayCollider::_Stab(const AABBNoLeafNode* node)
{
	...
	if (node->HasLeaf()) { STAB_PRIM(node->GetPrimitive()) }
	else
		_Stab(node->GetPos());

	if (ContactFound()) return;

	if (node->HasLeaf2()) { STAB_PRIM(node->GetPrimitive2()) }
	else
		_Stab(node->GetNeg());
}
...
void RayCollider::_Stab(const AABBTreeNode* node, Container& box_indices)
{
	...
	if (node->IsLeaf()) { box_indices.Add(node->GetPrimitives(), node->GetNbPrimitives()); }
	else
	{
...
void RayCollider::_UnboundedStab(const AABBCollisionNode* node)
{
	...
	if (node->IsLeaf()) { UNBOUNDED_STAB_PRIM(node->GetPrimitive()) }
	else
	{
...
void RayCollider::_UnboundedStab(const AABBNoLeafNode* node)
{
	...
	if (node->HasLeaf()) { UNBOUNDED_STAB_PRIM(node->GetPrimitive()) }
	else
		_UnboundedStab(node->GetPos());

	if (ContactFound()) return;

	if (node->HasLeaf2()) { UNBOUNDED_STAB_PRIM(node->GetPrimitive2()) }
	else
		_UnboundedStab(node->GetNeg());
}
