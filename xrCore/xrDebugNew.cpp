				 ...
				 const char* file, int line, const char* function, LPSTR assertion_info)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		if (!i) buffer += sprintf(buffer, "%sFATAL ERROR%s%s", endline, endline, endline);
	...
	{
		if (shared_str_initialized) Msg("stack trace:\n");
		...
		{
			if (shared_str_initialized) Msg("%s", g_stackTrace[i]);
...
{
#ifdef USE_BUG_TRAP
	if (!xr_FS || !FS.m_Flags.test(CLocatorAPI::flReady)) return;
...
void save_mini_dump(_EXCEPTION_POINTERS* pExceptionInfo)
{
	...
	if (hDll)
	{
		...
		if (pDump)
		{
			...
			__try
			{
				if (FS.path_exist("$logs$")) FS.update_path(szDumpPath, "$logs$", szDumpPath);
			}
			__except (EXCEPTION_EXECUTE_HANDLER)
			{
				string_path temp;
		...
		else { szResult = "DBGHELP.DLL too old"; }
	}
	else { szResult = "DBGHELP.DLL not found"; }
...
void format_message(LPSTR buffer, const u32& buffer_size)
{
	...
	sprintf(buffer, "[error][%lu]    : %s", error_code, (LPSTR)message);
	LocalFree(message);
}
