...
{
	if (!g_mem_alloc_gather_stats) return;

	if (::Random.randF() >= g_mem_alloc_gather_stats_frequency) return;

	//		OutputDebugStackTrace	("----------------------------------------------------");

	BuildStackTrace();

	if (g_stackTraceCount <= 2) return;
	...
	for (; I != E; ++I)
	{
		if ((*I).first != crc) break;

		if (xr_strcmp((*I).second.first, string)) continue;
