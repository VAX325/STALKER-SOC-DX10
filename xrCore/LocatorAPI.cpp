...
void _check_open_file(const shared_str& _fname)
{
	...
	if (it != g_open_files.end()) Log("file opened at least twice", _fname.c_str());
}
...
XRCORE_API void _dump_open_files(int mode)
{
	...
	else
	{
		...
		{
			_open_file& _of = *it;
			if (_of._reader == NULL) Msg("[%d] fname:%s", _of._used, _of._fn.c_str());
		}
	}
	if (bShow) Log("----total count=", g_open_files.size());
}
	...
	while (true)
	{
		...
		if ((dwType & (~CFS_CompressMark)) == ID)
		{
			...
			if (dwType & (~CFS_CompressMark))
			{
				BYTE* dest;
				unsigned dest_sz;
				if (g_temporary_stuff) g_temporary_stuff(src_data, dwSize, src_data);
				_decompressLZ(&dest, &dest_sz, src_data, dwSize);
				xr_free(src_data);
				return xr_new<CTempReader>(dest, dest_sz, 0);
			}
			else { return xr_new<CTempReader>(src_data, dwSize, 0); }
		}
		else
		{
...
void CLocatorAPI::ProcessArchive(LPCSTR _path, LPCSTR base_path)
{
	...
	if (g_temporary_stuff_subst) g_temporary_stuff = g_temporary_stuff_subst;
}
...
bool CLocatorAPI::Recurse(const char* path)
{
	...
	if (m_Flags.test(flNeedCheck))
	{
		...
		if (!ignore_name(sFile.name) && !ignore_path(full_path)) rec_files.push_back(sFile);
	...
	// insert self
	if (path && path[0]) Register(path, 0xffffffff, 0, 0, 0, 0, 0);
...
void CLocatorAPI::_initialize(u32 flags, LPCSTR target_folder, LPCSTR fs_name)
{
	...
	if (m_Flags.is(flScanAppRoot)) { append_path("$app_root$", Core.WorkingPath, 0, FALSE); }
	...
	{ // find nearest fs.ltx and set fs_root correctly
		...
		if (!pFSltx && m_Flags.is(flScanAppRoot)) pFSltx = r_open("$app_root$", fs_ltx);
		...
		if (!pFSltx)
		{
			...
			if (xr_strlen(tmpAppPath))
			{
				...
				if (strrchr(tmpAppPath, '\\')) *(strrchr(tmpAppPath, '\\') + 1) = 0;
	...
	// target folder
	if (m_Flags.is(flTargetFolderOnly)) { append_path("$target_folder$", target_folder, 0, TRUE); }
	else
	{
		/*
	...
	for (++I; I != files.end(); I++)
	{
		...
		if ((*end_symbol) != '\\')
		{
			...
			if (flags & FS_ClampExt)
				if (0 != strext(fname)) *strext(fname) = 0;
		}
		else
		{
			// folder
...
{
#ifdef DEBUG
	if (m_Flags.is(flCacheFiles)) check_cached_files(fname, desc, source_name);
#endif // DEBUG
...
#ifdef DEBUG
	if (R && m_Flags.is(flBuildCopy | flReady)) copy_file_to_build(R, source_name);
#endif // DEBUG

	if (m_Flags.test(flDumpFileActivity)) _register_open_file(R, fname);
