...
int heap_walk(HANDLE heap_handle, struct _heapinfo* _entry)
{
	...
	else
	{
		...
		{
			if (!HeapValidate(heap_handle, 0, _entry->_pentry)) return _HEAPBADNODE;
...
nextBlock:
		...
		{
			errflag = 0;
			if (!HeapWalk(heap_handle, &Entry)) errflag = 1;
		}
		...
		if (errflag == 1)
		{
			 ...
			 */
			if ((errval = GetLastError()) == ERROR_NO_MORE_ITEMS) { return _HEAPEND; }
	...
	if (Entry.wFlags & (PROCESS_HEAP_REGION | PROCESS_HEAP_UNCOMMITTED_RANGE)) { goto nextBlock; }

	_entry->_pentry = (int*)Entry.lpData;
	_entry->_size = Entry.cbData;
	if (Entry.wFlags & PROCESS_HEAP_ENTRY_BUSY) { _entry->_useflag = _USEDENTRY; }
	else { _entry->_useflag = _FREEENTRY; }
