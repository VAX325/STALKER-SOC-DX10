...
void PPM_CONTEXT::makeSuffix()
{
	...
	if (!NumStats)
	{
		...
		if (!Suffix->NumStats) { p1 = &(Suffix->oneState()); }
		else
		{
...
static inline void UpdateModel(PPM_CONTEXT* MinContext)
{
	...
	*pText++ = FSymbol;
	...
	if (FSuccessor)
	{
		if ((BYTE*)FSuccessor < UnitsStart) FSuccessor = CreateSuccessors(FALSE, p, MinContext);
	}
	else { FSuccessor = ReduceOrder(p, MinContext); }
