...
BOOL __stdcall RealAssert(DWORD dwOverrideOpts, LPCSTR szMsg, BOOL bAllowHalts)
{
	...
	if (DA_USEDEFAULTS == dwOverrideOpts) { dwOpts = g_DiagAssertOptions; }
	...
	LPSTR szRealLastErr;
	if (NULL != szFmtMsg) { szRealLastErr = szFmtMsg; }
	else { szRealLastErr = "**Last error code does not exist!!!!"; }

	// Get the module name.
	if (0 == GetModuleFileNameA(NULL, szModName, MAX_PATH)) { lstrcpy(szModName, "<unknown application>"); }
	...
	if (NULL != szFmtMsg) { LocalFree((LPVOID)szFmtMsg); }
	...
	if (DA_SHOWODS == (DA_SHOWODS & dwOpts)) { OutputDebugStringA(szBuff); }
	...
	if (TRUE == bAllowHalts) { uiType |= MB_ABORTRETRYIGNORE; }
	else { uiType |= MB_OK; }
	...
	if (DA_SHOWMSGBOX == (DA_SHOWMSGBOX & dwOpts))
	{
		...
		if (NULL != hWndParent) { hWndParent = GetLastActivePopup(hWndParent); }
	...
	if ((IDIGNORE == iRet) || (IDOK == iRet)) { return (FALSE); }
	if (IDRETRY == iRet)
	{
...
static DWORD __stdcall GetModBase(HANDLE hProcess, DWORD dwAddr)
{
	...
	if (g_cSym.SymGetModuleInfo(dwAddr, &stIHM)) { return (stIHM.BaseOfImage); }
	else
	{
...
#ifdef _DEBUG
			if (0 == dwRet) { TRACE("SymLoadModule failed : 0x%08X\n", GetLastError()); }
#endif // _DEBUG
...
static DWORD ConvertAddress(DWORD dwAddr, LPTSTR szOutBuff)
{
	...
	if (0 != g_cSym.SymGetModuleInfo(dwAddr, &stIHM))
	{
		...
		else { szName = stIHM.ImageName; }
		pCurrPos += wsprintf(pCurrPos, _T ( "%s: " ), szName);
	}
	else { pCurrPos += wsprintf(pCurrPos, _T ( "<unknown module>: " )); }

	// Get the function.
	DWORD dwDisp;
	if (0 != g_cSym.SymGetSymFromAddr(dwAddr, &dwDisp, pIHS))
	{
		if (0 == dwDisp) { pCurrPos += wsprintf(pCurrPos, _T ( "%s" ), pIHS->Name); }
		else { pCurrPos += wsprintf(pCurrPos, _T ( "%s + %d bytes" ), pIHS->Name, dwDisp); }
		...
		if (0 != g_cSym.SymGetLineFromAddr(dwAddr, &dwDisp, &stIHL))
		{
			...
			if (0 != dwDisp) { pCurrPos += wsprintf(pCurrPos, _T ( " + %d bytes" ), dwDisp); }
		}
	}
	else { pCurrPos += wsprintf(pCurrPos, _T ( "<unknown symbol>" )); }
...
#error( "Unknown machine!" )
		...
		for (DWORD i = 0; i < 512; i++)
		{
			...
			if (0 != stFrame.AddrPC.Offset) { vAddrs.push_back(stFrame.AddrPC.Offset); }
		}
		...
		for (loop = vAddrs.begin(); loop != vAddrs.end(); loop++)
		{
			...
			// DiagAssert.cpp
			if (FALSE == bSeenDiagAssert) { continue; }
			if (dwSizeLeft < dwSymSize) { break; }
