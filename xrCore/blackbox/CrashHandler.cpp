...
BOOL __stdcall SetCrashHandlerFilter(PFNCHFILTFN pFn)
{
	...
	else
	{
		...
		if (TRUE == IsBadCodePtr((FARPROC)pFn)) { return (FALSE); }
		g_pfnCallBack = pFn;

		// If a custom crash handler isn't already in use, enable
		// CrashHandlerExceptionFilter and save the original unhandled
		// exception filter.
		if (NULL == g_pfnOrigFilt) { g_pfnOrigFilt = SetUnhandledExceptionFilter(CrashHandlerExceptionFilter); }
	}
	return (TRUE);
}
...
LPCTSTR __stdcall GetFaultReason(EXCEPTION_POINTERS* pExPtrs)
{
	...
	__try
	{
		...
		if (NULL != dwTemp) { iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s" ), dwTemp); }
		else
		{
		...
		if (NULL == dwTemp) { iCurr += wsprintf(g_szBuff + iCurr, _T ( "<UNKNOWN>" )); }
		else
		{
									  ...
									  &dwDisp, pSym))
		{
			...
			{
				if (dwDisp > 0) { iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s()+%04d byte(s)" ), pSym->Name, dwDisp); }
...
BOOL __stdcall GetFaultReasonVB(EXCEPTION_POINTERS* pExPtrs, LPTSTR szBuff, UINT uiSize)
{
	...
	__try
	{
		...
		ASSERT(NULL != szRet);
		if (NULL == szRet) { return (NULL != szRet); }
...
#define CH_MACHINE IMAGE_FILE_MACHINE_I386
		...
		if (GSTSO_SYMBOL == (dwOpts & GSTSO_SYMBOL))
		{
			...
			if (TRUE == SymGetSymFromAddr((HANDLE)GetCurrentProcessId(), g_stFrame.AddrPC.Offset, &dwDisp, pSym))
			{
				...
				{
					if (dwDisp > 0) { iCurr += wsprintf(g_szBuff + iCurr, _T( "%s()"), pSym->Name, dwDisp); }
					else { iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s" ), pSym->Name); }
				}
			}
			else
