...
bool file_handle_internal(LPCSTR file_name, u32& size, int& file_handle)
{
	...
	{
		Sleep(1);
		if (open_internal(file_name, file_handle)) return (false);
	}
...
void IWriter::w_compressed(void* ptr, u32 count)
{
	...
	if (dest && dest_sz) w(dest, dest_sz);
	xr_free(dest);
}
...
IReader* IReader::open_chunk(u32 ID)
{
	...
	if (dwSize != 0)
	{
		...
		else { return new IReader(pointer(), dwSize, tell() + dwSize); }
	}
	else
		return 0;
};
