...
CInifile::CInifile(LPCSTR szFileName, BOOL ReadOnly, BOOL bLoad, BOOL SaveAtEnd)
{
	m_file_name[0] = 0;
	m_flags.zero();
	if (szFileName) strcpy_s(m_file_name, szFileName);
...
CInifile::~CInifile()
{
	...
	{
		if (!save_as()) Log("! Can't save inifile:", m_file_name);
	}
...
static void insert_item(CInifile::Sect* tgt, const CInifile::Item& I)
{
	...
	else { tgt->Data.insert(sect_it, I); }
}
...
void CInifile::Load(IReader* F, LPCSTR path)
{
	...
	while (!F->eof())
	{
		...
		if (semi_1 && (*(semi_1 + 1) == '/') && ((!semi) || (semi && (semi_1 < semi)))) { semi = semi_1; }
		...
		bool bReadOnly = m_flags.test(eReadOnly);
...
{
	// save if needed
	if (new_fname && new_fname[0]) strcpy_s(m_file_name, new_fname);
...
int CInifile::r_token(LPCSTR S, LPCSTR L, const xr_token* token_list)
{
	...
	for (int i = 0; token_list[i].name; i++)
		if (!stricmp(C, token_list[i].name)) return token_list[i].id;
	return 0;
}
...
BOOL CInifile::r_line(const shared_str& S, int L, const char** N, const char** V) { return r_line(*S, L, N, V); }
