...
void IPureServer::_Recieve(const void* data, u32 data_size, u32 param)
{
	...
	if (psNET_Flags.test(NETFLAG_LOG_SV_PACKETS))
	{
		...
		if (pSvNetLog) pSvNetLog->LogPacket(TimeGlobal(device_timer), &packet, TRUE);
	}
	//---------------------------------------
	...
	if (result) SendBroadcast(id, packet, result);
}
...
IPureServer::EConnect IPureServer::Connect(LPCSTR options)
{
	...
	if (!psNET_direct_connect)
	{
	   ...
	   // Create the IDirectPlay8Client object.
		...
		while (HostSuccess != S_OK && psNET_Port <= END_PORT)
		{
			...
			else { Msg("- IPureServer : created on port %d!", psNET_Port); }
		};
	...
	if (!psNET_direct_connect) BannedList_Load();
...
void IPureServer::Disconnect()
{
	...
	if (!psNET_direct_connect) BannedList_Save();
		...
		case DPN_MSGID_RECEIVE:
			{
				...
				else { MultipacketReciever::RecievePacket(pMsg->pReceiveData, pMsg->dwReceiveDataSize, m_sender); }
			}
			break;
...
void IPureServer::SendTo_Buf(ClientID id, void* data, u32 size, u32 dwFlags, u32 dwTimeout)
{
	...
	if (it != net_Players.end()) (*it)->MultipacketSender::SendPacket(data, size, dwFlags, dwTimeout);
}
...
void IPureServer::SendTo_LL(ClientID ID /*DPNID ID*/, void* data, u32 size, u32 dwFlags, u32 dwTimeout)
{
	...
	if (ID.value()) stats.dwBytesSended += size;
#endif
...
void IPureServer::OnCL_Connected(IClient* CL) { Msg("* Player '%s' connected.\n", CL->name.c_str()); }
void IPureServer::OnCL_Disconnected(IClient* CL) { Msg("* Player '%s' disconnected.\n", CL->name.c_str()); }
...
bool IPureServer::DisconnectAddress(const ip_address& Address)
{
	...
	for (u32 idx = 0; idx < net_Players.size(); ++idx)
	{
		...
		if (Address == ClAddress) { PlayersToDisconnect.push_back(net_Players[idx]); };
	};
...
IBannedClient* IPureServer::GetBannedClient(const ip_address& Address)
{
	for (u32 it = 0; it < BannedAddresses.size(); it++)
	{
		...
		if (pBClient->HAddr == Address) return pBClient;
	}
	return NULL;
};
...
LPCSTR IPureServer::GetBannedListName() { return "banned_list.ltx"; }
