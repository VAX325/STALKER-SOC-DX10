...
void INetQueue::Release()
{
	...
	u32 tmp_time = GetTickCount() - 60000;
	u32 size = unused.size();
	if ((LastTimeCreate < tmp_time) && (size > 32)) { xr_delete(ready.front()); }
	else
...
void IPureClient::_Recieve(const void* data, u32 data_size, u32 /*param*/)
{
	...
	else if (net_Connected == EnmConnectionCompleted)
	{
		...
		if (psNET_Flags.test(NETFLAG_LOG_CL_PACKETS))
		{
			...
			if (pClNetLog) pClNetLog->LogData(timeServer(), const_cast<void*>(data), data_size, TRUE);
		}
...
BOOL IPureClient::Connect(LPCSTR options)
{
	...
	if (!psNET_direct_connect && !strstr(options, "localhost")) { gen_auth_code(); }
	...
	if (!psNET_direct_connect)
	{
		...
		if (strchr(options, '/')) strncpy(server_name, options, strchr(options, '/') - options);
		...
		if (stricmp(server_name, "localhost") == 0)
		{
			...
			while (res != S_OK && c_port <= u32(psCL_Port + 100))
			{
				...
				else { Msg("- IPureClient : created on port %d!", c_port); }
			};
					...
					case DPN_MSGID_CONNECT_COMPLETE:
						{
							...
							{
								string256 ResStr = "";
								strncpy(ResStr, (char*)(pMsg->pvApplicationReplyData),
...
void IPureClient::SendTo_LL(void* data, u32 size, u32 dwFlags, u32 dwTimeout)
{
	...
	{
		if (!pClNetLog) pClNetLog = xr_new<INetLog>("logs\\net_cl_log.log", timeServer());
		if (pClNetLog) pClNetLog->LogData(timeServer(), data, size);
	}
